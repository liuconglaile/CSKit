...
#define StorageNSLog(FORMAT, ...) fprintf(stderr,"\n\n\n🍎🍎🍎方法:%s \n🍊🍊🍊行号:%d \n🍌🍌🍌内容:%s\n", __FUNCTION__, __LINE__, [[NSString stringWithFormat:FORMAT, ##__VA_ARGS__] UTF8String]);


#else // 开发模式
#define StorageNSLog(FORMAT, ...) nil
...
@implementation CSKVStorageItem
@end


@implementation CSKVStorage
{
...
- (BOOL)_dbClose {
    ...
    do {
        ...
        if (result == SQLITE_BUSY || result == SQLITE_LOCKED) {
            ...
            if (_errorLogsEnabled) {
                StorageNSLog(@"%s line:%d sqlite close failed (%d).", __FUNCTION__, __LINE__, result);
            }
        }
    } while (retry);
    _db = NULL;
...
- (BOOL)_dbExecute:(NSString *)sql {
    ...
    if (error) {
        if (_errorLogsEnabled) StorageNSLog(@"%s line:%d sqlite exec error (%d): %s", __FUNCTION__, __LINE__, result, error);
        sqlite3_free(error);
    }
...
- (sqlite3_stmt *)_dbPrepareStmt:(NSString *)sql {
    ...
    if (!stmt) {
        ...
        if (result != SQLITE_OK) {
            if (_errorLogsEnabled) StorageNSLog(@"%s line:%d sqlite stmt prepare error (%d): %s", __FUNCTION__, __LINE__, result, sqlite3_errmsg(_db));
            return NULL;
        }
...
- (BOOL)_dbSaveWithKey:(NSString *)key value:(NSData *)value fileName:(NSString *)fileName extendedData:(NSData *)extendedData {
    ...
    if (result != SQLITE_DONE) {
        if (_errorLogsEnabled) StorageNSLog(@"%s line:%d sqlite insert error (%d): %s", __FUNCTION__, __LINE__, result, sqlite3_errmsg(_db));
        return NO;
    }
    return YES;
}
...
- (BOOL)_dbUpdateAccessTimeWithKey:(NSString *)key {
    ...
    int result = sqlite3_step(stmt);
    if (result != SQLITE_DONE) {
        if (_errorLogsEnabled) StorageNSLog(@"%s line:%d sqlite update error (%d): %s", __FUNCTION__, __LINE__, result, sqlite3_errmsg(_db));
        return NO;
    }
    return YES;
}
...
- (BOOL)_dbDeleteItemWithKey:(NSString *)key {
    ...
    int result = sqlite3_step(stmt);
    if (result != SQLITE_DONE) {
        if (_errorLogsEnabled) StorageNSLog(@"%s line:%d db delete error (%d): %s", __FUNCTION__, __LINE__, result, sqlite3_errmsg(_db));
        return NO;
    }
    return YES;
}
...
- (BOOL)_dbDeleteItemWithKeys:(NSArray *)keys {
    ...
    if (result == SQLITE_ERROR) {
        if (_errorLogsEnabled) StorageNSLog(@"%s line:%d sqlite delete error (%d): %s", __FUNCTION__, __LINE__, result, sqlite3_errmsg(_db));
        return NO;
    }
    return YES;
}
...
- (CSKVStorageItem *)_dbGetItemFromStmt:(sqlite3_stmt *)stmt excludeInlineData:(BOOL)excludeInlineData {
    int i = 0;
    ...
    CSKVStorageItem *item = [CSKVStorageItem new];
...
- (CSKVStorageItem *)_dbGetItemWithKey:(NSString *)key excludeInlineData:(BOOL)excludeInlineData {
    NSString *sql = excludeInlineData ? @"select key, filename, size, modification_time, last_access_time, extended_data from manifest where key = ?1;" : @"select key, filename, size, inline_data, modification_time, last_access_time, extended_data from manifest where key = ?1;";
    sqlite3_stmt *stmt = [self _dbPrepareStmt:sql];
    if (!stmt) return nil;
    sqlite3_bind_text(stmt, 1, key.UTF8String, -1, NULL);
    
    CSKVStorageItem *item = nil;
    int result = sqlite3_step(stmt);
    if (result == SQLITE_ROW) {
        item = [self _dbGetItemFromStmt:stmt excludeInlineData:excludeInlineData];
    } else {
        if (result != SQLITE_DONE) {
            if (_errorLogsEnabled) StorageNSLog(@"%s line:%d sqlite query error (%d): %s", __FUNCTION__, __LINE__, result, sqlite3_errmsg(_db));
        }
    }
    return item;
}
...
- (NSMutableArray *)_dbGetItemWithKeys:(NSArray *)keys excludeInlineData:(BOOL)excludeInlineData {
    ...
    do {
        result = sqlite3_step(stmt);
        if (result == SQLITE_ROW) {
            CSKVStorageItem *item = [self _dbGetItemFromStmt:stmt excludeInlineData:excludeInlineData];
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"CSKVStorage init error" reason:@"Please use the designated initializer and pass the 'path' and 'type'." userInfo:nil];
    return [self initWithPath:@"" type:CSKVStorageTypeFile];
}

- (instancetype)initWithPath:(NSString *)path type:(CSKVStorageType)type {
    if (path.length == 0 || path.length > kPathLengthMax) {
        StorageNSLog(@"CSKVStorage init error: invalid path: [%@].", path);
        return nil;
    }
    if (type > CSKVStorageTypeMixed) {
        StorageNSLog(@"CSKVStorage init error: invalid type: %lu.", (unsigned long)type);
        return nil;
    }
    ...
    _trashQueue = dispatch_queue_create("com.ibireme.CSKit.cache.disk.trash", DISPATCH_QUEUE_SERIAL);
    ...
    if (![[NSFileManager defaultManager] createDirectoryAtPath:path
        ...
        ![[NSFileManager defaultManager] createDirectoryAtPath:[path stringByAppendingPathComponent:kTrashDirectoryName]
                                   withIntermediateDirectories:YES
                                                    attributes:nil
                                                         error:&error]) {
            StorageNSLog(@"CSKVStorage init error:%@", error);
            return nil;
        }
    ...
    if (![self _dbOpen] || ![self _dbInitialize]) {
        ...
        if (![self _dbOpen] || ![self _dbInitialize]) {
            [self _dbClose];
            StorageNSLog(@"CSKVStorage init error: fail to open sqlite db.");
            return nil;
        }
    }
...
- (BOOL)saveItemWithKey:(NSString *)key value:(NSData *)value filename:(NSString *)filename extendedData:(NSData *)extendedData {
    ...
    if (_type == CSKVStorageTypeFile && filename.length == 0) {
        return NO;
    }
    ...
    if (filename.length) {
        ...
        if (_type != CSKVStorageTypeSQLite) {
...
- (BOOL)removeItemForKey:(NSString *)key {
    ...
    switch (_type) {
        case CSKVStorageTypeSQLite: {
            return [self _dbDeleteItemWithKey:key];
        } break;
        case CSKVStorageTypeFile:
        case CSKVStorageTypeMixed: {
            NSString *filename = [self _dbGetFilenameWithKey:key];
            if (filename) {
                [self _fileDeleteWithName:filename];
            }
...
- (BOOL)removeItemsToFitSize:(int)maxSize {
    ...
    do {
        ...
        for (CSKVStorageItem *item in items) {
            if (total > maxSize) {
                if (item.filename) {
...
- (CSKVStorageItem *)getItemForKey:(NSString *)key {
    if (key.length == 0) return nil;
    CSKVStorageItem *item = [self _dbGetItemWithKey:key excludeInlineData:NO];
    if (item) {
...
- (CSKVStorageItem *)getItemInfoForKey:(NSString *)key {
    if (key.length == 0) return nil;
    CSKVStorageItem *item = [self _dbGetItemWithKey:key excludeInlineData:YES];
    return item;
}
