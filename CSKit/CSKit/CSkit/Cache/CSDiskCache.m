...
/**
 ...
 @return 字节数
 */
static int64_t _CSDiskSpaceFree() {
    NSError *error = nil;
...
/**
 全局缓存初始化
 */
static void _CSDiskCacheInitGlobal() {
...
/**
 ...
 @return 磁盘缓存
 */
static CSDiskCache *_CSDiskCacheGetGlobal(NSString *path) {
    if (path.length == 0) return nil;
    _CSDiskCacheInitGlobal();
...
/**
 ...
 @param cache 磁盘缓存
 */
static void _CSDiskCacheSetGlobal(CSDiskCache *cache) {
...
- (void)_trimToFreeDiskSpace:(NSUInteger)targetFreeDiskSpace {
    ...
    int64_t diskFreeBytes = _CSDiskSpaceFree();
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"CSDiskCache init error" reason:@"CSDiskCache must be initialized with a path. Use 'initWithPath:' or 'initWithPath:inlineThreshold:' instead." userInfo:nil];
...
- (instancetype)initWithPath:(NSString *)path
    ...
    CSDiskCache *globalCache = _CSDiskCacheGetGlobal(path);
    if (globalCache) return globalCache;
    
    CSKVStorageType type;
    if (threshold == 0) {
        type = CSKVStorageTypeFile;
    } else if (threshold == NSUIntegerMax) {
        type = CSKVStorageTypeSQLite;
    } else {
        type = CSKVStorageTypeMixed;
    }
    
    CSKVStorage *kv = [[CSKVStorage alloc] initWithPath:path type:type];
    if (!kv) return nil;
...
- (id<NSCoding>)objectForKey:(NSString *)key {
    if (!key) return nil;
    Lock();
    CSKVStorageItem *item = [_kv getItemForKey:key];
    Unlock();
    ...
    if (object && item.extendedData) {
        [CSDiskCache setExtendedData:item.extendedData toObject:object];
    }
    return object;
}
...
- (void)setObject:(id<NSCoding>)object forKey:(NSString *)key {
    ...
    NSData *extendedData = [CSDiskCache getExtendedDataFromObject:object];
    NSData *value = nil;
    ...
    if (!value) return;
    NSString *filename = nil;
    if (_kv.type != CSKVStorageTypeSQLite) {
