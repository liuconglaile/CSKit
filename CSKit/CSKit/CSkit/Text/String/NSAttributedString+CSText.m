...
@implementation NSAttributedString (CSText)
...
- (CSTextShadow *)textShadow {
    return [self textShadowAtIndex:0];
}

- (CSTextShadow *)textShadowAtIndex:(NSUInteger)index {
    return [self attribute:CSTextShadowAttributeName atIndex:index];
}

- (CSTextShadow *)textInnerShadow {
    return [self textInnerShadowAtIndex:0];
}

- (CSTextShadow *)textInnerShadowAtIndex:(NSUInteger)index {
    return [self attribute:CSTextInnerShadowAttributeName atIndex:index];
}

- (CSTextDecoration *)textUnderline {
    return [self textUnderlineAtIndex:0];
}

- (CSTextDecoration *)textUnderlineAtIndex:(NSUInteger)index {
    return [self attribute:CSTextUnderlineAttributeName atIndex:index];
}

- (CSTextDecoration *)textStrikethrough {
    return [self textStrikethroughAtIndex:0];
}

- (CSTextDecoration *)textStrikethroughAtIndex:(NSUInteger)index {
    return [self attribute:CSTextStrikethroughAttributeName atIndex:index];
}

- (CSTextBorder *)textBorder {
    return [self textBorderAtIndex:0];
}

- (CSTextBorder *)textBorderAtIndex:(NSUInteger)index {
    return [self attribute:CSTextBorderAttributeName atIndex:index];
}

- (CSTextBorder *)textBackgroundBorder {
    return [self textBackgroundBorderAtIndex:0];
}

- (CSTextBorder *)textBackgroundBorderAtIndex:(NSUInteger)index {
    return [self attribute:CSTextBackedStringAttributeName atIndex:index];
}
...
- (CGAffineTransform)textGlyphTransformAtIndex:(NSUInteger)index {
    NSValue *value = [self attribute:CSTextGlyphTransformAttributeName atIndex:index];
...
- (NSString *)plainTextForRange:(NSRange)range {
    ...
    [self enumerateAttribute:CSTextBackedStringAttributeName inRange:range options:kNilOptions usingBlock:^(id value, NSRange range, BOOL *stop) {
        CSTextBackedString *backed = value;
...
+ (NSMutableAttributedString *)attachmentStringWithContent:(id)content
    ...
    NSMutableAttributedString *atr = [[NSMutableAttributedString alloc] initWithString:CSTextAttachmentToken];
    
    CSTextAttachment *attach = [CSTextAttachment new];
...
+ (NSMutableAttributedString *)attachmentStringWithContent:(id)content
    ...
    switch (alignment) {
        case CSTextVerticalAlignmentTop: {
        ...
        case CSTextVerticalAlignmentCenter: {
        ...
        case CSTextVerticalAlignmentBottom: {
...
+ (NSMutableAttributedString *)attachmentStringWithEmojiImage:(UIImage *)image
    ...
    if (image.images.count > 1) {
        hasAnim = YES;
    } else if ([image conformsToProtocol:@protocol(CSAnimatedImage)]) {
        id <CSAnimatedImage> ani = (id)image;
        if (ani.animatedImageFrameCount > 1) hasAnim = YES;
    }
    
    CGFloat ascent = CSEmojiGetAscentWithFontSize(fontSize);
    CGFloat descent = CSEmojiGetDescentWithFontSize(fontSize);
    CGRect bounding = CSEmojiGetGlyphBoundingRectWithFontSize(fontSize);
    ...
    CSTextAttachment *attachment = [CSTextAttachment new];
    ...
    if (hasAnim) {
        CSAnimatedImageView *view = [CSAnimatedImageView new];
        view.frame = bounding;
        view.image = image;
...
- (BOOL)canDrawWithUIKit {
    ...
    dispatch_once(&onceToken, ^{
        ...
        [failSet addObject:CSTextShadowAttributeName];
        [failSet addObject:CSTextInnerShadowAttributeName];
        [failSet addObject:CSTextUnderlineAttributeName];
        [failSet addObject:CSTextStrikethroughAttributeName];
        [failSet addObject:CSTextBorderAttributeName];
        [failSet addObject:CSTextBackgroundBorderAttributeName];
        [failSet addObject:CSTextBlockBorderAttributeName];
        [failSet addObject:CSTextAttachmentAttributeName];
        [failSet addObject:CSTextHighlightAttributeName];
        [failSet addObject:CSTextGlyphTransformAttributeName];
    });
...
@implementation NSMutableAttributedString (CSText)
...
- (void)setTextBackedString:(CSTextBackedString *)textBackedString range:(NSRange)range {
    [self setAttribute:CSTextBackedStringAttributeName value:textBackedString range:range];
}

- (void)setTextBinding:(CSTextBinding *)textBinding range:(NSRange)range {
    [self setAttribute:CSTextBindingAttributeName value:textBinding range:range];
}

- (void)setTextShadow:(CSTextShadow *)textShadow range:(NSRange)range {
    [self setAttribute:CSTextShadowAttributeName value:textShadow range:range];
}

- (void)setTextInnerShadow:(CSTextShadow *)textInnerShadow range:(NSRange)range {
    [self setAttribute:CSTextInnerShadowAttributeName value:textInnerShadow range:range];
}

- (void)setTextUnderline:(CSTextDecoration *)textUnderline range:(NSRange)range {
    [self setAttribute:CSTextUnderlineAttributeName value:textUnderline range:range];
}

- (void)setTextStrikethrough:(CSTextDecoration *)textStrikethrough range:(NSRange)range {
    [self setAttribute:CSTextStrikethroughAttributeName value:textStrikethrough range:range];
}

- (void)setTextBorder:(CSTextBorder *)textBorder range:(NSRange)range {
    [self setAttribute:CSTextBorderAttributeName value:textBorder range:range];
}

- (void)setTextBackgroundBorder:(CSTextBorder *)textBackgroundBorder range:(NSRange)range {
    [self setAttribute:CSTextBackgroundBorderAttributeName value:textBackgroundBorder range:range];
}

- (void)setTextAttachment:(CSTextAttachment *)textAttachment range:(NSRange)range {
    [self setAttribute:CSTextAttachmentAttributeName value:textAttachment range:range];
}

- (void)setTextHighlight:(CSTextHighlight *)textHighlight range:(NSRange)range {
    [self setAttribute:CSTextHighlightAttributeName value:textHighlight range:range];
}

- (void)setTextBlockBorder:(CSTextBorder *)textBlockBorder range:(NSRange)range {
    [self setAttribute:CSTextBlockBorderAttributeName value:textBlockBorder range:range];
}

- (void)setTextRubyAnnotation:(CSTextRubyAnnotation *)ruby range:(NSRange)range {
    if (kiOS8Later) {
...
- (void)setTextGlyphTransform:(CGAffineTransform)textGlyphTransform range:(NSRange)range {
    ...
    [self setAttribute:CSTextGlyphTransformAttributeName value:value range:range];
}
...
- (void)setTextHighlightRange:(NSRange)range
              ...
              backgroundColor:(UIColor *)backgroundColor
                    ...
                    tapAction:(CSTextAction)tapAction
              longPressAction:(CSTextAction)longPressAction {
    CSTextHighlight *highlight = [CSTextHighlight highlightWithBackgroundColor:backgroundColor];
...
+ (NSArray *)allDiscontinuousAttributeKeys {
    ...
    dispatch_once(&onceToken, ^{
        keys = @[(id)kCTSuperscriptAttributeName,
                 ...
                 CSTextBackedStringAttributeName,
                 CSTextBindingAttributeName,
                 CSTextAttachmentAttributeName].mutableCopy;
        if (kiOS8Later) {
