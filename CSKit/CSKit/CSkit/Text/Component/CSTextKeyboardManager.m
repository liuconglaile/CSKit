...
static int _CSTextKeyboardViewFrameObserverKey;
NSString *const kCSTextKeyboardViewFrameObserverFrame = @"frame";
NSString *const kCSTextKeyboardViewFrameObserverCenter = @"center";
NSString *const kCSTextKeyboardViewFrameObserverBounds = @"bounds";
NSString *const kCSTextKeyboardViewFrameObserverTransform = @"transform";

/// Observer for view's frame/bounds/center/transform
@interface _CSTextKeyboardViewFrameObserver : NSObject
...
@implementation _CSTextKeyboardViewFrameObserver {
...
- (void)addToKeyboardView:(UIView *)keyboardView {
    ...
    if (_keyboardView) {
        ...
        objc_setAssociatedObject(_keyboardView, &_CSTextKeyboardViewFrameObserverKey, nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    _keyboardView = keyboardView;
    if (keyboardView) {
        [self addFrameObserver];
    }
    objc_setAssociatedObject(keyboardView, &_CSTextKeyboardViewFrameObserverKey, self, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (void)removeFrameObserver {
    [_keyboardView removeObserver:self forKeyPath:kCSTextKeyboardViewFrameObserverFrame];
    [_keyboardView removeObserver:self forKeyPath:kCSTextKeyboardViewFrameObserverCenter];
    [_keyboardView removeObserver:self forKeyPath:kCSTextKeyboardViewFrameObserverBounds];
    [_keyboardView removeObserver:self forKeyPath:kCSTextKeyboardViewFrameObserverTransform];
    _keyboardView = nil;
}

- (void)addFrameObserver {
    if (!_keyboardView) return;
    [_keyboardView addObserver:self forKeyPath:kCSTextKeyboardViewFrameObserverFrame options:kNilOptions context:NULL];
    [_keyboardView addObserver:self forKeyPath:kCSTextKeyboardViewFrameObserverCenter options:kNilOptions context:NULL];
    [_keyboardView addObserver:self forKeyPath:kCSTextKeyboardViewFrameObserverBounds options:kNilOptions context:NULL];
    [_keyboardView addObserver:self forKeyPath:kCSTextKeyboardViewFrameObserverTransform options:kNilOptions context:NULL];
}
...
+ (instancetype)observerForView:(UIView *)keyboardView {
    ...
    return objc_getAssociatedObject(keyboardView, &_CSTextKeyboardViewFrameObserverKey);
}
...
@implementation CSTextKeyboardManager{
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"CSTextKeyboardManager init error" reason:@"Use 'defaultManager' to get instance." userInfo:nil];
    return [super init];
}
...
- (void)_initFrameObserver {
    ...
    _CSTextKeyboardViewFrameObserver *observer = [_CSTextKeyboardViewFrameObserver observerForView:keyboardView];
    if (!observer) {
        observer = [_CSTextKeyboardViewFrameObserver new];
...
+ (instancetype)defaultManager {
    static CSTextKeyboardManager *mgr = nil;
...
- (void)_notifyAllObservers {
    ...
    CSTextKeyboardTransition trans = {0};
    ...
    if (!CGRectEqualToRect(trans.toFrame, _fromFrame)) {
        for (id<CSTextKeyboardObserver> observer in _observers.copy) {
