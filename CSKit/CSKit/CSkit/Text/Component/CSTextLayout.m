...
static inline CGSize CSTextClipCGSize(CGSize size) {
    if (size.width > CSTextContainerMaxSize.width) size.width = CSTextContainerMaxSize.width;
    if (size.height > CSTextContainerMaxSize.height) size.height = CSTextContainerMaxSize.height;
    return size;
}
...
/**
 有时CoreText可能会在iOS7中将CGColor转换为'kCTForegroundColorAttributeName'属性的UIColor.
 这应该是CoreText的错误,可能会导致崩溃.这里有一个解决方法.
 */
static CGColorRef CSTextGetCGColor(CGColorRef color) {
...
@implementation CSTextLinePositionSimpleModifier
- (void)modifyLines:(NSArray *)lines fromText:(NSAttributedString *)text inContainer:(CSTextContainer *)container {
    if (container.verticalForm) {
        for (NSUInteger i = 0, max = lines.count; i < max; i++) {
            CSTextLine *line = lines[i];
...
- (id)copyWithZone:(NSZone *)zone {
    CSTextLinePositionSimpleModifier *one = [self.class new];
...
@implementation CSTextContainer {
    @package
    BOOL _readonly; ///< used only in CSTextLayout.implementation
    ...
    CSTextTruncationType _truncationType;
    NSAttributedString *_truncationToken;
    id<CSTextLinePositionModifier> _linePositionModifier;
}
...
+ (instancetype)containerWithSize:(CGSize)size insets:(UIEdgeInsets)insets {
    CSTextContainer *one = [self new];
    one.size = CSTextClipCGSize(size);
    one.insets = insets;
    return one;
}
...
- (id)copyWithZone:(NSZone *)zone {
    CSTextContainer *one = [self.class new];
...
reason:@"Cannot change the property of the 'container' in 'CSTextLayout'." userInfo:nil]; \
return; \
} \
dispatch_semaphore_wait(_lock, DISPATCH_TIME_FOREVER); \
__VA_ARGS__; \
...
- (void)setSize:(CGSize)size {
    Setter(if(!_path) _size = CSTextClipCGSize(size));
}
...
- (CSTextTruncationType)truncationType {
    Getter(CSTextTruncationType type = _truncationType) return type;
}

- (void)setTruncationType:(CSTextTruncationType)truncationType {
...
- (void)setLinePositionModifier:(id<CSTextLinePositionModifier>)linePositionModifier {
    Setter(_linePositionModifier = [(NSObject *)linePositionModifier copy]);
}

- (id<CSTextLinePositionModifier>)linePositionModifier {
    Getter(id<CSTextLinePositionModifier> m = _linePositionModifier) return m;
}
...
@interface CSTextLayout ()

@property (nonatomic, readwrite) CSTextContainer *container;
...
@property (nonatomic, readwrite) CSTextLine *truncatedLine;
...
+ (CSTextLayout *)layoutWithContainer:(CSTextContainer *)container text:(NSAttributedString *)text {
    return [self layoutWithContainer:container text:text range:NSMakeRange(0, text.length)];
}

+ (CSTextLayout *)layoutWithContainer:(CSTextContainer *)container text:(NSAttributedString *)text range:(NSRange)range {
    CSTextLayout *layout = NULL;
    ...
    layout = [[CSTextLayout alloc] _init];
    layout.text = text;
    ...
    if (container.path == nil && container.exclusionPaths.count == 0) {
        ...
        if (needFixLayoutSizeBug) {
            ...
            if (container.isVerticalForm) {
                rect.size.width = CSTextContainerMaxSize.width;
            } else {
                rect.size.height = CSTextContainerMaxSize.height;
            }
        }
    ...
    ctFrame = CTFramesetterCreateFrame(ctSetter, CSCFRangeFromNSRange(range), cgPath, (CFTypeRef)frameAttrs);
    ...
    for (NSUInteger i = 0; i < lineCount; i++) {
        ...
        CSTextLine *line = [CSTextLine lineWithCTLine:ctLine position:position vertical:isVerticalForm];
    ...
    if (rowCount > 0) {
        if (maximumNumberOfRows > 0) {
            if (rowCount > maximumNumberOfRows) {
                ...
                do {
                    CSTextLine *line = lines.lastObject;
                    if (!line) break;
        ...
        CSTextLine *lastLine = lines.lastObject;
    ...
    visibleRange = CSNSRangeFromCFRange(CTFrameGetVisibleStringRange(ctFrame));
    if (needTruncation) {
        CSTextLine *lastLine = lines.lastObject;
        ...
        if (container.truncationType != CSTextTruncationTypeNone) {
            ...
            if (container.truncationToken) {
                ...
                truncationToken = [[NSAttributedString alloc] initWithString:CSTextTruncationToken attributes:attrs];
                truncationTokenLine = CTLineCreateWithAttributedString((CFAttributedStringRef)truncationToken);
            }
            if (truncationTokenLine) {
                CTLineTruncationType type = kCTLineTruncationEnd;
                if (container.truncationType == CSTextTruncationTypeStart) {
                    type = kCTLineTruncationStart;
                } else if (container.truncationType == CSTextTruncationTypeMiddle) {
                ...
                if (ctLastLineExtend) {
                    ...
                    if (ctTruncatedLine) {
                        truncatedLine = [CSTextLine lineWithCTLine:ctTruncatedLine position:lastLine.position vertical:isVerticalForm];
    ...
    if (isVerticalForm) {
        NSCharacterSet *rotateCharset = CSTextVerticalFormRotateCharacterSet();
        NSCharacterSet *rotateMoveCharset = CSTextVerticalFormRotateAndMoveCharacterSet();
        
        void (^lineBlock)(CSTextLine *) = ^(CSTextLine *line){
            ...
            for (NSUInteger r = 0; r < runCount; r++) {
                ...
                CSTextRunGlyphDrawMode prevMode = CSTextRunGlyphDrawModeHorizontal;
                ...
                for (NSUInteger g = 0; g < glyphCount; g++) {
                    ...
                    CSTextRunGlyphDrawMode mode = glyphRotateMove ? CSTextRunGlyphDrawModeVerticalRotateMove : (glyphRotate ? CSTextRunGlyphDrawModeVerticalRotate : CSTextRunGlyphDrawModeHorizontal);
                    if (g == 0) {
                        prevMode = mode;
                    } else if (mode != prevMode) {
                        CSTextRunGlyphRange *aRange = [CSTextRunGlyphRange rangeWithRange:NSMakeRange(prevIdx, g - prevIdx) drawMode:prevMode];
                ...
                if (prevIdx < glyphCount) {
                    CSTextRunGlyphRange *aRange = [CSTextRunGlyphRange rangeWithRange:NSMakeRange(prevIdx, glyphCount - prevIdx) drawMode:prevMode];
                    [runRanges addObject:aRange];
                }
                
            }
        };
        for (CSTextLine *line in lines) {
            lineBlock(line);
        }
    ...
    if (visibleRange.length > 0) {
        ...
        void (^block)(NSDictionary *attrs, NSRange range, BOOL *stop) = ^(NSDictionary *attrs, NSRange range, BOOL *stop) {
            if (attrs[CSTextHighlightAttributeName]) layout.containsHighlight = YES;
            if (attrs[CSTextBlockBorderAttributeName]) layout.needDrawBlockBorder = YES;
            if (attrs[CSTextBackgroundBorderAttributeName]) layout.needDrawBackgroundBorder = YES;
            if (attrs[CSTextShadowAttributeName] || attrs[NSShadowAttributeName]) layout.needDrawShadow = YES;
            if (attrs[CSTextUnderlineAttributeName]) layout.needDrawUnderline = YES;
            if (attrs[CSTextAttachmentAttributeName]) layout.needDrawAttachment = YES;
            if (attrs[CSTextInnerShadowAttributeName]) layout.needDrawInnerShadow = YES;
            if (attrs[CSTextStrikethroughAttributeName]) layout.needDrawStrikethrough = YES;
            if (attrs[CSTextBoundingStrokeAttributedName]) layout.needDrawBorder = YES;
        };
    ...
    for (NSUInteger i = 0, max = lines.count; i < max; i++) {
        ...
        if (line.attachments.count > 0) {
            ...
            for (CSTextAttachment *attachment in line.attachments) {
...
+ (NSArray *)layoutWithContainers:(NSArray *)containers text:(NSAttributedString *)text range:(NSRange)range {
    ...
    for (NSUInteger i = 0, max = containers.count; i < max; i++) {
        CSTextContainer *container = containers[i];
        CSTextLayout *layout = [self layoutWithContainer:container text:text range:range];
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    NSData *textData = [CSTextArchiver archivedDataWithRootObject:_text];
...
- (id)initWithCoder:(NSCoder *)aDecoder {
    ...
    NSAttributedString *text = [CSTextUnarchiver unarchiveObjectWithData:textData];
    CSTextContainer *container = [aDecoder decodeObjectForKey:@"container"];
 ...
 */
- (CTRunRef)_runForLine:(CSTextLine *)line position:(CSTextPosition *)position {
    if (!line || !position) return NULL;
    CFArrayRef runs = CTLineGetGlyphRuns(line.CTLine);
    for (NSUInteger i = 0, max = CFArrayGetCount(runs); i < max; i++) {
        CTRunRef run = CFArrayGetValueAtIndex(runs, i);
        CFRange range = CTRunGetStringRange(run);
        if (position.affinity == CSTextAffinityBackward) {
...
/**
 ...
 next:  right position
 */
- (BOOL)_insideComposedCharacterSequences:(CSTextLine *)line position:(NSUInteger)position block:(void (^)(CGFloat left, CGFloat right, NSUInteger prev, NSUInteger next))block {
 ...
 */
- (BOOL)_insideEmoji:(CSTextLine *)line position:(NSUInteger)position block:(void (^)(CGFloat left, CGFloat right, NSUInteger prev, NSUInteger next))block {
    if (!line) return NO;
    CFArrayRef runs = CTLineGetGlyphRuns(line.CTLine);
...
/**
 ...
 @return YES if RTL.
 */
- (BOOL)_isRightToLeftInLine:(CSTextLine *)line atPoint:(CGPoint)point {
    if (!line) return NO;
    // get write direction
    BOOL RTL = NO;
    CFArrayRef runs = CTLineGetGlyphRuns(line.CTLine);
 ...
 */
- (CSTextRange *)_correctedRangeWithEdge:(CSTextRange *)range {
    NSRange visibleRange = self.visibleRange;
    CSTextPosition *start = range.start;
    CSTextPosition *end = range.end;
    
    if (start.offset == visibleRange.location && start.affinity == CSTextAffinityBackward) {
        start = [CSTextPosition positionWithOffset:start.offset affinity:CSTextAffinityForward];
    }
    
    if (end.offset == visibleRange.location + visibleRange.length && start.affinity == CSTextAffinityForward) {
        end = [CSTextPosition positionWithOffset:end.offset affinity:CSTextAffinityBackward];
    }
    
    if (start != range.start || end != range.end) {
        range = [CSTextRange rangeWithStart:start end:end];
    }
    return range;
}
...
- (NSUInteger)rowIndexForLine:(NSUInteger)line {
    ...
    return ((CSTextLine *)_lines[line]).row;
}
...
- (NSUInteger)lineIndexForPoint:(CGPoint)point {
    ...
    for (NSUInteger i = lineIdx0; i <= lineIdx1; i++) {
        CGRect bounds = ((CSTextLine *)_lines[i]).bounds;
...
- (CGFloat)offsetForTextPosition:(NSUInteger)position lineIndex:(NSUInteger)lineIndex {
    ...
    CSTextLine *line = _lines[lineIndex];
...
- (CSTextPosition *)closestPositionToPoint:(CGPoint)point {
    BOOL isVertical = _container.verticalForm;
    ...
    if (position <= _visibleRange.location) {
        return [CSTextPosition positionWithOffset:_visibleRange.location affinity:CSTextAffinityForward];
    } else if (position >= _visibleRange.location + _visibleRange.length) {
        return [CSTextPosition positionWithOffset:_visibleRange.location + _visibleRange.length affinity:CSTextAffinityBackward];
    }
    
    CSTextAffinity finalAffinity = CSTextAffinityForward;
    BOOL finalAffinityDetected = NO;
    
    // binding range
    NSRange bindingRange;
    CSTextBinding *binding = [_text attribute:CSTextBindingAttributeName atIndex:position longestEffectiveRange:&bindingRange inRange:NSMakeRange(0, _text.length)];
    if (binding && bindingRange.length > 0) {
        NSUInteger headLineIdx = [self lineIndexForPosition:[CSTextPosition positionWithOffset:bindingRange.location]];
        NSUInteger tailLineIdx = [self lineIndexForPosition:[CSTextPosition positionWithOffset:bindingRange.location + bindingRange.length affinity:CSTextAffinityBackward]];
            ...
            if (left != CGFLOAT_MAX && right != CGFLOAT_MAX) {
                if (_container.isVerticalForm) {
                    if (fabs(point.y - left) < fabs(point.y - right)) {
                        ...
                        finalAffinity = CSTextAffinityForward;
                    } else {
                        position = bindingRange.location + bindingRange.length;
                        finalAffinity = CSTextAffinityBackward;
                    }
                } else {
    ...
    if (line.range.length == 0) {
        ...
        return [CSTextPosition positionWithOffset:line.range.location affinity:behind ? CSTextAffinityBackward:CSTextAffinityForward];
    }
    ...
    if (line.range.length <= 2) {
        ...
        if (CSTextIsLinebreakString(str)) { // an empty line ("\r", "\n", "\r\n")
            return [CSTextPosition positionWithOffset:line.range.location];
        }
    }
    ...
    if (position >= line.range.location + line.range.length - 1) {
        if (position > line.range.location) {
            ...
            if (CSTextIsLinebreakChar(c1)) {
                position--;
                if (position > line.range.location) {
                    unichar c0 = [_text.string characterAtIndex:position - 1];
                    if (CSTextIsLinebreakChar(c0)) {
                        position--;
                    }
                }
            }
    ...
    if (position == line.range.location) {
        return [CSTextPosition positionWithOffset:position];
    }
    if (position == line.range.location + line.range.length) {
        return [CSTextPosition positionWithOffset:position affinity:CSTextAffinityBackward];
    }
    ...
    if (!finalAffinityDetected) {
        ...
        if (ofs != CGFLOAT_MAX) {
            ...
            if (position >= line.range.location + line.range.length) {
                finalAffinity = RTL ? CSTextAffinityForward : CSTextAffinityBackward;
            } else if (position <= line.range.location) {
                finalAffinity = RTL ? CSTextAffinityBackward : CSTextAffinityForward;
            } else {
                finalAffinity = (ofs < (isVertical ? point.y : point.x) && !RTL) ? CSTextAffinityForward : CSTextAffinityBackward;
            }
        }
    }
    
    return [CSTextPosition positionWithOffset:position affinity:finalAffinity];
}

- (CSTextPosition *)positionForPoint:(CGPoint)point
                         oldPosition:(CSTextPosition *)oldPosition
                       otherPosition:(CSTextPosition *)otherPosition {
    if (!oldPosition || !otherPosition) {
        return oldPosition;
    }
    CSTextPosition *newPos = [self closestPositionToPoint:point];
    ...
    if (_container.isVerticalForm) {
        if ([oldPosition compare:otherPosition] == NSOrderedAscending) { // search backward
            CSTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionUp offset:1];
            if (range) return range.start;
        } else { // search forward
            CSTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionDown offset:1];
            if (range) return range.end;
        }
    } else {
        if ([oldPosition compare:otherPosition] == NSOrderedAscending) { // search backward
            CSTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionLeft offset:1];
            if (range) return range.start;
        } else { // search forward
            CSTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionRight offset:1];
            if (range) return range.end;
        }
    }
...
- (CSTextRange *)textRangeAtPoint:(CGPoint)point {
    ...
    CSTextPosition *pos = [self closestPositionToPoint:point];
    if (!pos) return nil;
    ...
    if (_container.verticalForm) {
        CSTextRange *range = [self textRangeByExtendingPosition:pos inDirection:(rect.origin.y >= point.y && !RTL) ? UITextLayoutDirectionUp:UITextLayoutDirectionDown offset:1];
        return range;
    } else {
        CSTextRange *range = [self textRangeByExtendingPosition:pos inDirection:(rect.origin.x >= point.x && !RTL) ? UITextLayoutDirectionLeft:UITextLayoutDirectionRight offset:1];
        return range;
    }
}

- (CSTextRange *)closestTextRangeAtPoint:(CGPoint)point {
    CSTextPosition *pos = [self closestPositionToPoint:point];
    if (!pos) return nil;
    ...
    CSTextRange *range = [self textRangeByExtendingPosition:pos inDirection:direction offset:1];
    return range;
}

- (CSTextRange *)textRangeByExtendingPosition:(CSTextPosition *)position {
    ...
    if (position.offset == visibleStart) {
        return [CSTextRange rangeWithRange:NSMakeRange(position.offset, 0)];
    } else if (position.offset == visibleEnd) {
        return [CSTextRange rangeWithRange:NSMakeRange(position.offset, 0) affinity:CSTextAffinityBackward];
    }
    
    // binding range
    NSRange tRange;
    CSTextBinding *binding = [_text attribute:CSTextBindingAttributeName atIndex:position.offset longestEffectiveRange:&tRange inRange:_visibleRange];
    if (binding && tRange.length > 0 && tRange.location < position.offset) {
        return [CSTextRange rangeWithRange:tRange];
    }
    ...
    if (lineIndex != NSNotFound) {
        ...
        if (emoji || seq) {
            return [CSTextRange rangeWithRange:NSMakeRange(_prev, _next - _prev)];
        }
    }
    ...
    if (position.offset > visibleStart && position.offset < visibleEnd) {
        ...
        if ((c0 == '\r') && position.offset < visibleEnd) {
            ...
            if (c1 == '\n') {
                return [CSTextRange rangeWithStart:[CSTextPosition positionWithOffset:position.offset - 1] end:[CSTextPosition positionWithOffset:position.offset + 1]];
            }
        }
        if (CSTextIsLinebreakChar(c0) && position.affinity == CSTextAffinityBackward) {
            NSString *str = [_text.string substringToIndex:position.offset];
            NSUInteger len = CSTextLinebreakTailLength(str);
            return [CSTextRange rangeWithStart:[CSTextPosition positionWithOffset:position.offset - len] end:[CSTextPosition positionWithOffset:position.offset]];
        }
    }
    
    return [CSTextRange rangeWithRange:NSMakeRange(position.offset, 0) affinity:position.affinity];
}
...
- (CSTextRange *)textRangeByExtendingPosition:(CSTextPosition *)position
    ...
    if (!forwardMove && position.offset == visibleStart) {
        return [CSTextRange rangeWithRange:NSMakeRange(_visibleRange.location, 0)];
    } else if (forwardMove && position.offset == visibleEnd) {
        return [CSTextRange rangeWithRange:NSMakeRange(position.offset, 0) affinity:CSTextAffinityBackward];
    }
    
    // extend from position
    CSTextRange *fromRange = [self textRangeByExtendingPosition:position];
    if (!fromRange) return nil;
    CSTextRange *allForward = [CSTextRange rangeWithStart:fromRange.start end:[CSTextPosition positionWithOffset:visibleEnd]];
    CSTextRange *allBackward = [CSTextRange rangeWithStart:[CSTextPosition positionWithOffset:visibleStart] end:fromRange.end];
    ...
    if (verticalMove) { // up/down in text layout
        ...
        CSTextLine *mostLeftLine = nil, *mostRightLine = nil;
        ...
        CSTextLine *insideLine = _lines[insideIndex];
        NSUInteger pos;
        if (isVerticalForm) {
        ...
        CSTextPosition *extPos;
        if (afinityEdge) {
            if (pos == insideLine.range.location + insideLine.range.length) {
                NSString *subStr = [_text.string substringWithRange:insideLine.range];
                NSUInteger lineBreakLen = CSTextLinebreakTailLength(subStr);
                extPos = [CSTextPosition positionWithOffset:pos - lineBreakLen];
            } else {
                extPos = [CSTextPosition positionWithOffset:pos];
            }
        } else {
            extPos = [CSTextPosition positionWithOffset:pos];
        }
        CSTextRange *ext = [self textRangeByExtendingPosition:extPos];
        if (!ext) return nil;
        if (forwardMove) {
            return [CSTextRange rangeWithStart:fromRange.start end:ext.end];
        } else {
            return [CSTextRange rangeWithStart:ext.start end:fromRange.end];
        }
        
    } else { // left/right in text layout
        CSTextPosition *toPosition = [CSTextPosition positionWithOffset:position.offset + (forwardMove ? offset : -offset)];
        if (toPosition.offset <= visibleStart) return allBackward;
        else if (toPosition.offset >= visibleEnd) return allForward;
        
        CSTextRange *toRange = [self textRangeByExtendingPosition:toPosition];
        ...
        return [CSTextRange rangeWithRange:NSMakeRange(start, end - start)];
    }
}
...
- (CGRect)firstRectForRange:(CSTextRange *)range {
    ...
    CSTextLine *startLine = _lines[startLineIndex];
    CSTextLine *endLine = _lines[endLineIndex];
    NSMutableArray *lines = [NSMutableArray new];
    for (NSUInteger i = startLineIndex; i <= startLineIndex; i++) {
        CSTextLine *line = _lines[i];
    ...
    if (_container.verticalForm) {
        if (lines.count == 1) {
            ...
            if (top > bottom) CS_SWAP(top, bottom);
        ...
        if (lines.count == 1) {
            ...
            if (left > right) CS_SWAP(left, right);
...
- (CGRect)rectForRange:(CSTextRange *)range {
    ...
    CGRect rectUnion = ((CSTextSelectionRect *)rects.firstObject).rect;
    for (NSUInteger i = 1; i < rects.count; i++) {
        CSTextSelectionRect *rect = rects[i];
        rectUnion = CGRectUnion(rectUnion, rect.rect);
    }
    return rectUnion;
}

- (NSArray *)selectionRectsForRange:(CSTextRange *)range {
    range = [self _correctedRangeWithEdge:range];
    ...
    if (startLineIndex > endLineIndex) CS_SWAP(startLineIndex, endLineIndex);
    CSTextLine *startLine = _lines[startLineIndex];
    CSTextLine *endLine = _lines[endLineIndex];
    ...
    CSTextSelectionRect *start = [CSTextSelectionRect new];
    if (isVertical) {
    ...
    CSTextSelectionRect *end = [CSTextSelectionRect new];
    if (isVertical) {
    ...
    if (startLine.row == endLine.row) { // same row
        if (offsetStart > offsetEnd) CS_SWAP(offsetStart, offsetEnd);
        CSTextSelectionRect *rect = [CSTextSelectionRect new];
        if (isVertical) {
        ...
        CSTextSelectionRect *topRect = [CSTextSelectionRect new];
        ...
        CSTextSelectionRect *bottomRect = [CSTextSelectionRect new];
...
- (NSArray *)selectionRectsWithoutStartAndEndForRange:(CSTextRange *)range {
...
- (NSArray *)selectionRectsWithOnlyStartAndEndForRange:(CSTextRange *)range {
    NSMutableArray *rects = [self selectionRectsForRange:range].mutableCopy;
    for (NSInteger i = 0, max = rects.count; i < max; i++) {
        CSTextSelectionRect *rect = rects[i];
        if (!rect.containsStart && !rect.containsEnd) {
...
typedef NS_OPTIONS(NSUInteger, CSTextDecorationType) {
    CSTextDecorationTypeUnderline     = 1 << 0,
    CSTextDecorationTypeStrikethrough = 1 << 1,
};

typedef NS_OPTIONS(NSUInteger, CSTextBoundingStrokeType) {
    CSTextBoundingStrokeTypeBackgound = 1 << 0,
    CSTextBoundingStrokeTypeNormal    = 1 << 1,
};

static CGRect CSTextMergeRectInSameLine(CGRect rect1, CGRect rect2, BOOL isVertical) {
    if (isVertical) {
...
static void CSTextGetRunsMaxMetric(CFArrayRef runs, CGFloat *xHeight, CGFloat *underlinePosition, CGFloat *lineThickness) {
...
static void CSTextDrawRun(CSTextLine *line, CTRunRef run, CGContextRef context, CGSize size, BOOL isVertical, NSArray *runRanges, CGFloat verticalOffset) {
    CGAffineTransform runTextMatrix = CTRunGetTextMatrix(run);
    BOOL runTextMatrixIsID = CGAffineTransformIsIdentity(runTextMatrix);
    
    CFDictionaryRef runAttrs = CTRunGetAttributes(run);
    NSValue *glyphTransformValue = CFDictionaryGetValue(runAttrs, (__bridge const void *)(CSTextGlyphTransformAttributeName));
    if (!isVertical && !glyphTransformValue) { // draw run
        ...
        fillColor = CSTextGetCGColor(fillColor);
        ...
        CGContextSaveGState(context); {
            ...
            if (isVertical) {
                ...
                for (CSTextRunGlyphRange *oneRange in runRanges) {
                    NSRange range = oneRange.glyphRangeInRun;
                    NSUInteger rangeMax = range.location + range.length;
                    CSTextRunGlyphDrawMode mode = oneRange.drawMode;
                    ...
                    for (NSUInteger g = range.location; g < rangeMax; g++) {
                        CGContextSaveGState(context); {
                            ...
                            if (mode) { // CJK glyph, need rotated
                                ...
                                if (mode == CSTextRunGlyphDrawModeVerticalRotateMove) {
                                    x += w;
                                    y += w;
                                }
...
static void CSTextSetLinePatternInContext(CSTextLineStyle style, CGFloat width, CGFloat phase, CGContextRef context){
    ...
    } else if (pattern == CSTextLineStylePatternCircleDot) {
...
static void CSTextDrawBorderRects(CGContextRef context, CGSize size, CSTextBorder *border, NSArray *rects, BOOL isVertical) {
    if (rects.count == 0) return;
    
    CSTextShadow *shadow = border.shadow;
    if (shadow.color) {
        CGContextSaveGState(context);
    ...
    if (border.strokeColor && border.lineStyle > 0 && border.strokeWidth > 0) {
        ...
        CSTextSetLinePatternInContext(border.lineStyle, border.strokeWidth, 0, context);
        CGFloat inset = -border.strokeWidth * 0.5;
        if ((border.lineStyle & 0xFF) == CSTextLineStyleThick) {
            inset *= 2;
        ...
        //------------------------- second line ------------------------------//
        if ((border.lineStyle & 0xFF) == CSTextLineStyleDouble) {
            CGContextSaveGState(context);
...
static void CSTextDrawLineStyle(CGContextRef context, CGFloat length, CGFloat lineWidth, CSTextLineStyle style, CGPoint position, CGColorRef color, BOOL isVertical) {
    ...
    CGContextSaveGState(context); {
        if (isVertical) {
            ...
            w = (styleBase == CSTextLineStyleThick ? lineWidth * 2 : lineWidth);
            ...
            CSTextSetLinePatternInContext(style, lineWidth, position.y, context);
            CGContextSetLineWidth(context, w);
            if (styleBase == CSTextLineStyleSingle) {
                CGContextMoveToPoint(context, x, y1);
                CGContextAddLineToPoint(context, x, y2);
                CGContextStrokePath(context);
            } else if (styleBase == CSTextLineStyleThick) {
                CGContextMoveToPoint(context, x, y1);
                CGContextAddLineToPoint(context, x, y2);
                CGContextStrokePath(context);
            } else if (styleBase == CSTextLineStyleDouble) {
            ...
            CGContextSetStrokeColorWithColor(context, color);
            CSTextSetLinePatternInContext(style, lineWidth, position.x, context);
            CGContextSetLineWidth(context, w);
            if (styleBase == CSTextLineStyleSingle) {
...
static void CSTextDrawText(CSTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    CGContextSaveGState(context); {
        ...
        for (NSUInteger l = 0, lMax = lines.count; l < lMax; l++) {
            ...
            for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
                ...
                CSTextDrawRun(line, run, context, size, isVertical, lineRunRanges[r], verticalOffset);
            }
...
static void CSTextDrawBlockBorder(CSTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    CGContextSaveGState(context);
    CGContextTranslateCTM(context, point.x, point.y);
    ...
    for (NSInteger l = 0, lMax = lines.count; l < lMax; l++) {
        ...
        for (NSInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            CSTextBorder *border = attrs[CSTextBlockBorderAttributeName];
            if (!border) continue;
            
            NSUInteger lineStartIndex = line.index;
            while (lineStartIndex > 0) {
                if (((CSTextLine *)lines[lineStartIndex - 1]).row == line.row) lineStartIndex--;
                else break;
            }
            
            CGRect unionRect = CGRectZero;
            NSUInteger lineStartRow = ((CSTextLine *)lines[lineStartIndex]).row;
            NSUInteger lineContinueIndex = lineStartIndex;
            NSUInteger lineContinueRow = lineStartRow;
            do {
                CSTextLine *one = lines[lineContinueIndex];
                ...
                CSTextLine *next = lines[lineContinueIndex + 1];
                if (next.row != lineContinueRow) {
                    CSTextBorder *nextBorder = [layout.text attribute:CSTextBlockBorderAttributeName atIndex:next.range.location];
            ...
            CSTextDrawBorderRects(context, size, border, @[[NSValue valueWithCGRect:unionRect]], isVertical);
...
static void CSTextDrawBorder(CSTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, CSTextBoundingStrokeType type, BOOL (^cancel)(void)) {
    CGContextSaveGState(context);
    CGContextTranslateCTM(context, point.x, point.y);
    ...
    NSArray *lines = layout.lines;
    NSString *borderKey = (type == CSTextBoundingStrokeTypeNormal ? CSTextBoundingStrokeAttributedName : CSTextBackgroundBorderAttributeName);
    ...
    for (NSInteger l = 0, lMax = lines.count; l < lMax; l++) {
        ...
        for (NSInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            NSDictionary *attrs = (id)CTRunGetAttributes(run);
            CSTextBorder *border = attrs[borderKey];
            if (!border) continue;
            ...
            for (NSInteger ll = l; ll < lMax; ll++) {
                ...
                for (NSInteger rr = (ll == l) ? r : 0, rrMax = CFArrayGetCount(iRuns); rr < rrMax; rr++) {
                    ...
                    CSTextBorder *iBorder = iAttrs[borderKey];
                    ...
                    if (isVertical) {
                        CS_SWAP(iRunPosition.x, iRunPosition.y);
            ...
            for (NSInteger re = 0, reMax = runRects.count; re < reMax; re++) {
                ...
                if (isVertical) {
                    if (fabs(rect.origin.x - curRect.origin.x) < 1) {
                        curRect = CSTextMergeRectInSameLine(rect, curRect, isVertical);
                    } else {
            ...
            CSTextDrawBorderRects(context, size, border, drawRects, isVertical);
...
static void CSTextDrawDecoration(CSTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, CSTextDecorationType type, BOOL (^cancel)(void)) {
    NSArray *lines = layout.lines;
    ...
    for (NSUInteger l = 0, lMax = layout.lines.count; l < lMax; l++) {
        ...
        for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            NSDictionary *attrs = (id)CTRunGetAttributes(run);
            CSTextDecoration *underline = attrs[CSTextUnderlineAttributeName];
            CSTextDecoration *strikethrough = attrs[CSTextStrikethroughAttributeName];
            
            BOOL needDrawUnderline = NO, needDrawStrikethrough = NO;
            if ((type & CSTextDecorationTypeUnderline) && underline.style > 0) {
                needDrawUnderline = YES;
            }
            if ((type & CSTextDecorationTypeStrikethrough) && strikethrough.style > 0) {
            ...
            if (CSTextIsLinebreakString(runStr)) continue; // may need more checks...
            
            CGFloat xHeight, underlinePosition, lineThickness;
            CSTextGetRunsMaxMetric(runs, &xHeight, &underlinePosition, &lineThickness);
                ...
                while (shadow) {
                    ...
                    CGContextSaveGState(context); {
                        ...
                        CGContextSaveGState(context); {
                            ...
                            CSTextDrawLineStyle(context, length, thickness, underline.style, underlineStart, color, isVertical);
                        } CGContextRestoreGState(context);
                    } CGContextRestoreGState(context);
                    shadow = shadow.subShadow;
                }
            ...
            if (needDrawStrikethrough) {
                ...
                CSTextDrawLineStyle(context, length, thickness, strikethrough.style, strikethroughStart, color, isVertical);
            }
        }
    }
    CGContextRestoreGState(context);
...
static void CSTextDrawAttachment(CSTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, UIView *targetView, CALayer *targetLayer, BOOL (^cancel)(void)) {
    
    BOOL isVertical = layout.container.verticalForm;
    CGFloat verticalOffset = isVertical ? (size.width - layout.container.size.width) : 0;
    
    for (NSUInteger i = 0, max = layout.attachments.count; i < max; i++) {
        CSTextAttachment *a = layout.attachments[i];
        ...
        rect = CSCGRectFitWithContentMode(rect, asize, a.contentMode);
        rect = CGRectPixelRound(rect);
        rect = CGRectStandardize(rect);
...
static void CSTextDrawShadow(CSTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    ...
    CGContextSaveGState(context); {
        ...
        for (NSUInteger l = 0, lMax = layout.lines.count; l < lMax; l++) {
            ...
            for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
                ...
                NSDictionary *attrs = (id)CTRunGetAttributes(run);
                CSTextShadow *shadow = attrs[CSTextShadowAttributeName];
                CSTextShadow *nsShadow = [CSTextShadow shadowWithNSShadow:attrs[NSShadowAttributeName]]; // NSShadow compatible
                if (nsShadow) {
...
static void CSTextDrawInnerShadow(CSTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    CGContextSaveGState(context);
    CGContextTranslateCTM(context, point.x, point.y);
    CGContextTranslateCTM(context, 0, size.height);
    CGContextScaleCTM(context, 1, -1);
    ...
    for (NSUInteger l = 0, lMax = lines.count; l < lMax; l++) {
        ...
        for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            CGContextSetTextPosition(context, linePosX, linePosY);
            NSDictionary *attrs = (id)CTRunGetAttributes(run);
            CSTextShadow *shadow = attrs[CSTextInnerShadowAttributeName];
            while (shadow) {
                if (!shadow.color) {
                    shadow = shadow.subShadow;
                    continue;
...
static void CSTextDrawDebug(CSTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, CSTextDebugOption *op) {
    UIGraphicsPushContext(context);
    CGContextSaveGState(context);
    CGContextTranslateCTM(context, point.x, point.y);
    CGContextSetLineWidth(context, 1.0 / CSScreenScale());
...
- (void)drawInContext:(CGContextRef)context
                ...
                point:(CGPoint)point
                 view:(UIView *)view
                layer:(CALayer *)layer
                debug:(CSTextDebugOption *)debug
               cancel:(BOOL (^)(void))cancel{
    @autoreleasepool {
        if (self.needDrawBlockBorder && context) {
            if (cancel && cancel()) return;
            CSTextDrawBlockBorder(self, context, size, point, cancel);
        }
        if (self.needDrawBackgroundBorder && context) {
            if (cancel && cancel()) return;
            CSTextDrawBorder(self, context, size, point, CSTextBoundingStrokeTypeBackgound, cancel);
        }
        if (self.needDrawShadow && context) {
            if (cancel && cancel()) return;
            CSTextDrawShadow(self, context, size, point, cancel);
        }
        if (self.needDrawUnderline && context) {
            if (cancel && cancel()) return;
            CSTextDrawDecoration(self, context, size, point, CSTextDecorationTypeUnderline, cancel);
        }
        if (self.needDrawText && context) {
            if (cancel && cancel()) return;
            CSTextDrawText(self, context, size, point, cancel);
        }
        if (self.needDrawAttachment && (context || view || layer)) {
            if (cancel && cancel()) return;
            CSTextDrawAttachment(self, context, size, point, view, layer, cancel);
        }
        if (self.needDrawInnerShadow && context) {
            if (cancel && cancel()) return;
            CSTextDrawInnerShadow(self, context, size, point, cancel);
        }
        if (self.needDrawStrikethrough && context) {
            if (cancel && cancel()) return;
            CSTextDrawDecoration(self, context, size, point, CSTextDecorationTypeStrikethrough, cancel);
        }
        if (self.needDrawBorder && context) {
            if (cancel && cancel()) return;
            CSTextDrawBorder(self, context, size, point, CSTextBoundingStrokeTypeNormal, cancel);
        }
        if (debug.needDrawDebug && context) {
            if (cancel && cancel()) return;
            CSTextDrawDebug(self, context, size, point, debug);
        }
    }
}
...
- (void)removeAttachmentFromViewAndLayer {
    CSAssertMainThread();
    for (CSTextAttachment *a in self.attachments) {
