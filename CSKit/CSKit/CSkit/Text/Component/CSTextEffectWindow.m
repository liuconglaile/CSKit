...
@implementation CSTextEffectWindow

+ (instancetype)sharedWindow {
    static CSTextEffectWindow *one = nil;
    if (one == nil) {
        // iOS 9 compatible
...
- (CSTextDirection)_keyboardDirection {
    CGRect keyboardFrame = [CSTextKeyboardManager defaultManager].keyboardFrame;
    keyboardFrame = [[CSTextKeyboardManager defaultManager] convertRect:keyboardFrame toView:self];
    if (CGRectIsNull(keyboardFrame) || CGRectIsEmpty(keyboardFrame)) return CSTextDirectionNone;
    
    if (CGRectGetMinY(keyboardFrame) == 0 &&
        CGRectGetMinX(keyboardFrame) == 0 &&
        CGRectGetMaxX(keyboardFrame) == CGRectGetWidth(self.frame))
        return CSTextDirectionTop;
    ...
    if (CGRectGetMaxX(keyboardFrame) == CGRectGetWidth(self.frame) &&
        ...
        return CSTextDirectionRight;
    
    if (CGRectGetMaxY(keyboardFrame) == CGRectGetHeight(self.frame) &&
        CGRectGetMinX(keyboardFrame) == 0 &&
        CGRectGetMaxX(keyboardFrame) == CGRectGetWidth(self.frame))
        return CSTextDirectionBottom;
    
    if (CGRectGetMinX(keyboardFrame) == 0 &&
        CGRectGetMinY(keyboardFrame) == 0 &&
        CGRectGetMaxY(keyboardFrame) == CGRectGetHeight(self.frame))
        return CSTextDirectionLeft;
    
    return CSTextDirectionNone;
}
...
- (CGPoint)_correctedCenter:(CGPoint)center forMagnifier:(CSTextMagnifier *)mag rotation:(CGFloat)rotation {
    ...
    if (degree <= 1 || degree >= 7) { //top
        if (mag.type == CSTextMagnifierTypeCaret) {
            if (center.y < caretExt)
                center.y = caretExt;
        } else if (mag.type == CSTextMagnifierTypeRanged) {
 ...
 */
- (CGFloat)_updateMagnifier:(CSTextMagnifier *)mag {
    UIApplication *app = [UIApplication sharedExtensionApplication];
    if (!app) return 0;
    ...
    CGAffineTransform trans = CSCGAffineTransformGetFromViews(hostView, self);
    ...
    for (UIWindow *window in windows) {
        ...
        CGContextConcatCTM(context, CSCGAffineTransformGetFromViews(window, self));
...
- (void)showMagnifier:(CSTextMagnifier *)mag {
    ...
    mag.transform = trans;
    mag.center = center;
    if (mag.type == CSTextMagnifierTypeRanged) {
        mag.alpha = 0;
    }
    NSTimeInterval time = mag.type == CSTextMagnifierTypeCaret ? 0.08 : 0.1;
...
- (void)hideMagnifier:(CSTextMagnifier *)mag {
    ...
    CGFloat rotation = [self _updateMagnifier:mag];
    CGPoint center = [self convertPoint:mag.hostPopoverCenter fromViewOrWindow:mag.hostView];
    NSTimeInterval time = mag.type == CSTextMagnifierTypeCaret ? 0.20 : 0.15;
    [UIView animateWithDuration:time delay:0 options:UIViewAnimationOptionCurveEaseInOut | UIViewAnimationOptionAllowUserInteraction | UIViewAnimationOptionBeginFromCurrentState animations:^{
...
- (void)_updateSelectionGrabberDot:(CSSelectionGrabberDot *)dot selection:(CSTextSelectionView *)selection{
