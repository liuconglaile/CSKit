...
NSString *const CSTextViewTextDidBeginEditingNotification = @"CSTextViewTextDidBeginEditing";
NSString *const CSTextViewTextDidChangeNotification = @"CSTextViewTextDidChange";
NSString *const CSTextViewTextDidEndEditingNotification = @"CSTextViewTextDidEndEditing";


typedef NS_ENUM (NSUInteger, CSTextGrabberDirection) {
    kStart = 1,
    kEnd   = 2,
};

typedef NS_ENUM(NSUInteger, CSTextMoveDirection) {
    kLeft   = 1,
    kTop    = 2,
    kRight  = 3,
    kBottom = 4,
...
@interface _CSTextViewUndoObject : NSObject
@property (nonatomic, strong) NSAttributedString *text;
@property (nonatomic, assign) NSRange selectedRange;
@end
@implementation _CSTextViewUndoObject
+ (instancetype)objectWithText:(NSAttributedString *)text range:(NSRange)range {
    _CSTextViewUndoObject *obj = [self new];
...
@interface CSTextView () <UIScrollViewDelegate, UIAlertViewDelegate, CSTextDebugTarget, CSTextKeyboardObserver> {
    
    CSTextRange *_selectedTextRange; /// nonnull
    CSTextRange *_markedTextRange;
    
    __weak id<CSTextViewDelegate> _outerDelegate;
    ...
    CSTextContainer *_innerContainer; ///< nonnull, inner text container
    CSTextLayout *_innerLayout; ///< inner text layout, the text in this layout is longer than `_innerText` by appending '\n'
    
    CSTextContainerView *_containerView; ///< nonnull
    CSTextSelectionView *_selectionView; ///< nonnull
    CSTextMagnifier *_magnifierCaret; ///< nonnull
    CSTextMagnifier *_magnifierRanged; ///< nonnull
    ...
    CSTextHighlight *_highlight; ///< highlight attribute in `_highlightRange`
    CSTextLayout *_highlightLayout; ///< when _state.showingHighlight=YES, this layout should be displayed
    CSTextRange *_trackingRange; ///< the range in _innerLayout, may out of _innerText.
    ...
    struct {
        unsigned int trackingGrabber : 2;       ///< CSTextGrabberDirection, current tracking grabber
        ...
        unsigned int touchMoved : 3;            ///< CSTextMoveDirection, move direction after touch began
...
@synthesize selectedTextRange = _selectedTextRange;  //copy nonnull (CSTextRange*)
@synthesize markedTextRange = _markedTextRange;      //readonly     (CSTextRange*)
...
- (void)_commitUpdate {
    ...
    [[CSTransaction_Utility transactionWithTarget:self selector:@selector(_updateIfNeeded)] commit];
#else
    [self _update];
#endif
}
...
- (void)_updateLayout {
    ...
    [text removeAttribute:CSTextBorderAttributeName range:NSMakeRange(_innerText.length, 1)];
    [text removeAttribute:CSTextBackgroundBorderAttributeName range:NSMakeRange(_innerText.length, 1)];
    ...
    _innerLayout = [CSTextLayout layoutWithContainer:_innerContainer text:text];
...
- (void)_updateSelectionView {
    ...
    [[CSTextEffectWindow sharedWindow] hideSelectionDot:_selectionView];
    if (!_innerLayout) return;
    
    NSMutableArray *allRects = [NSMutableArray new];
    BOOL containsDot = NO;
    
    CSTextRange *selectedRange = _selectedTextRange;
    ...
    [allRects enumerateObjectsUsingBlock:^(CSTextSelectionRect *rect, NSUInteger idx, BOOL *stop) {
    ...
    if (!_state.firstShowDot && containsDot) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.02 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [[CSTextEffectWindow sharedWindow] showSelectionDot:_selectionView];
        });
    }
    [[CSTextEffectWindow sharedWindow] showSelectionDot:_selectionView];
...
- (void)_commitPlaceholderUpdate {
    ...
    [[CSTransaction_Utility transactionWithTarget:self selector:@selector(_updatePlaceholderIfNeeded)] commit];
#else
...
- (void)_updatePlaceholder {
    ...
    if (_placeholderAttributedText.length > 0) {
        CSTextContainer *container = _innerContainer.copy;
        container.size = self.bounds.size;
        container.truncationType = CSTextTruncationTypeEnd;
        container.truncationToken = nil;
        CSTextLayout *layout = [CSTextLayout layoutWithContainer:container text:_placeholderAttributedText];
...
- (void)_updateTextRangeByTrackingCaret {
    ...
    CSTextPosition *newPos = [_innerLayout closestPositionToPoint:trackingPoint];
    if (newPos) {
        ...
        CSTextRange *newRange = [CSTextRange rangeWithRange:NSMakeRange(newPos.offset, 0) affinity:newPos.affinity];
...
- (void)_updateTextRangeByTrackingGrabber {
    ...
    CSTextPosition *position = [_innerLayout positionForPoint:magPoint
    ...
    if (position) {
        ...
        if ((NSUInteger)position.offset > _innerText.length) {
            position = [CSTextPosition positionWithOffset:_innerText.length];
        }
        CSTextRange *newRange = [CSTextRange rangeWithStart:(isStart ? position : _selectedTextRange.start)
...
- (void)_updateTextRangeByTrackingPreSelect {
    if (!_state.trackingTouch) return;
    CSTextRange *newRange = [self _getClosestTokenRangeAtPoint:_trackingPoint];
    _trackingRange = newRange;
}
...
- (void)_showMagnifierCaret {
    ...
    if (_state.showingMagnifierRanged) {
        ...
        [[CSTextEffectWindow sharedWindow] hideMagnifier:_magnifierRanged];
    }
    ...
    if (!_state.showingMagnifierCaret) {
        ...
        [[CSTextEffectWindow sharedWindow] showMagnifier:_magnifierCaret];
    } else {
        [[CSTextEffectWindow sharedWindow] moveMagnifier:_magnifierCaret];
    }
}
...
- (void)_showMagnifierRanged {
    ...
    if (_state.showingMagnifierCaret) {
        ...
        [[CSTextEffectWindow sharedWindow] hideMagnifier:_magnifierCaret];
    }
    ...
    if (lineIndex < _innerLayout.lines.count) {
        CSTextLine *line = _innerLayout.lines[lineIndex];
        CGRect lineRect = [self _convertRectFromLayout:line.bounds];
        if (_verticalForm) {
            magPoint.x = CS_CLAMP(magPoint.x, CGRectGetMinX(lineRect), CGRectGetMaxX(lineRect));
        } else {
            magPoint.y = CS_CLAMP(magPoint.y, CGRectGetMinY(lineRect), CGRectGetMaxY(lineRect));
        }
        ...
        if (!_state.showingMagnifierRanged) {
            ...
            [[CSTextEffectWindow sharedWindow] showMagnifier:_magnifierRanged];
        } else {
            [[CSTextEffectWindow sharedWindow] moveMagnifier:_magnifierRanged];
        }
    }
}
...
- (void)_showMenu {
    ...
    if (_selectionView.caretVisible) {
        ...
        CSTextSelectionRect *sRect = _selectionView.selectionRects.firstObject;
        rect = sRect.rect;
        ...
        CSTextKeyboardManager *mgr = [CSTextKeyboardManager defaultManager];
...
- (void)_showHighlightAnimated:(BOOL)animated {
    ...
    if (!_highlightLayout) {
        ...
        _highlightLayout = [CSTextLayout layoutWithContainer:_innerContainer text:hiText];
...
- (void)_scrollRangeToVisible:(CSTextRange *)range {
    if (!range) return;
...
- (void)_keyboardChanged {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        if ([CSTextKeyboardManager defaultManager].keyboardVisible) {
            [self _scrollRangeToVisible:_selectedTextRange];
        } else {
...
- (void)_startLongPressTimer {
    ...
    _longPressTimer = [NSTimer timerWithTimeInterval:kLongPressMinimumDuration
                                              target:[CSWeakProxy proxyWithTarget:self]
...
- (void)_trackDidLongPress {
    ...
    if (_state.showingHighlight) {
        ...
        if (_highlight.longPressAction) {
            ...
            CGRect rect = [_innerLayout rectForRange:[CSTextRange rangeWithRange:_highlightRange]];
            rect = [self _convertRectFromLayout:rect];
        ...
        if (_state.trackingTouch) {
            if (_state.trackingGrabber) {
                ...
                if (newPos) {
                    ...
                    _trackingRange = [CSTextRange rangeWithRange:NSMakeRange(newPos.offset, 0) affinity:newPos.affinity];
                    [self _updateSelectionView];
                }
                [self _hideMenu];
...
- (void)_fixSelectionDot {
    ...
    CGPoint origin = [self convertPoint:CGPointZero toViewOrWindow:[CSTextEffectWindow sharedWindow]];
...
- (CSTextRange *)_getClosestTokenRangeAtPosition:(CSTextPosition *)position {
    position = [self _correctedTextPosition:position];
    if (!position) return nil;
    CSTextRange *range = nil;
    if (_tokenizer) {
    ...
    if (!range || range.asRange.length == 0) {
        ...
        CSTextRange *extStart = [_innerLayout textRangeByExtendingPosition:range.start];
        CSTextRange *extEnd = [_innerLayout textRangeByExtendingPosition:range.end];
        if (extStart && extEnd) {
            NSArray *arr = [@[extStart.start, extStart.end, extEnd.start, extEnd.end] sortedArrayUsingSelector:@selector(compare:)];
            range = [CSTextRange rangeWithStart:arr.firstObject end:arr.lastObject];
        }
    }
    
    range = [self _correctedTextRange:range];
    if (range.asRange.length == 0) {
        range = [CSTextRange rangeWithRange:NSMakeRange(0, _innerText.length)];
    }
...
/// Try to get the character range/position with word granularity from the tokenizer.
- (CSTextRange *)_getClosestTokenRangeAtPoint:(CGPoint)point {
    point = [self _convertPointToLayout:point];
    CSTextRange *touchRange = [_innerLayout closestTextRangeAtPoint:point];
    touchRange = [self _correctedTextRange:touchRange];
    
    if (_tokenizer && touchRange) {
        CSTextRange *encEnd = (id)[_tokenizer rangeEnclosingPosition:touchRange.end withGranularity:UITextGranularityWord inDirection:UITextStorageDirectionBackward];
        CSTextRange *encStart = (id)[_tokenizer rangeEnclosingPosition:touchRange.start withGranularity:UITextGranularityWord inDirection:UITextStorageDirectionForward];
        if (encEnd && encStart) {
            NSArray *arr = [@[encEnd.start, encEnd.end, encStart.start, encStart.end] sortedArrayUsingSelector:@selector(compare:)];
            touchRange = [CSTextRange rangeWithStart:arr.firstObject end:arr.lastObject];
        }
    }
    
    if (touchRange) {
        CSTextRange *extStart = [_innerLayout textRangeByExtendingPosition:touchRange.start];
        CSTextRange *extEnd = [_innerLayout textRangeByExtendingPosition:touchRange.end];
        if (extStart && extEnd) {
            NSArray *arr = [@[extStart.start, extStart.end, extEnd.start, extEnd.end] sortedArrayUsingSelector:@selector(compare:)];
            touchRange = [CSTextRange rangeWithStart:arr.firstObject end:arr.lastObject];
        }
    ...
    if (!touchRange) touchRange = [CSTextRange defaultRange];
    
    if (_innerText.length && touchRange.asRange.length == 0) {
        touchRange = [CSTextRange rangeWithRange:NSMakeRange(0, _innerText.length)];
    }
...
- (CSTextHighlight *)_getHighlightAtPoint:(CGPoint)point range:(NSRangePointer)range {
    if (!_highlightable || !_innerLayout.containsHighlight) return nil;
    point = [self _convertPointToLayout:point];
    CSTextRange *textRange = [_innerLayout textRangeAtPoint:point];
    ...
    CSTextHighlight *highlight = [text attribute:CSTextHighlightAttributeName
                                         atIndex:startIndex
...
- (CGFloat)_getMagnifierRangedOffset {
    ...
    CSTextPosition *position = [_innerLayout closestPositionToPoint:magPoint];
    NSUInteger lineIndex = [_innerLayout lineIndexForPosition:position];
    if (lineIndex < _innerLayout.lines.count) {
        CSTextLine *line = _innerLayout.lines[lineIndex];
        if (_verticalForm) {
            magPoint.x = CS_CLAMP(magPoint.x, line.left, line.right);
            return magPoint.x - line.position.x + kMagnifierRangedPopoverOffset;
        } else {
            magPoint.y = CS_CLAMP(magPoint.y, line.top, line.bottom);
...
/// Return a CSTextMoveDirection from `_touchBeganPoint` to `_trackingPoint`.
...
- (BOOL)_isTextPositionValid:(CSTextPosition *)position {
    if (!position) return NO;
    if (position.offset < 0) return NO;
    if (position.offset > _innerText.length) return NO;
    if (position.offset == 0 && position.affinity == CSTextAffinityBackward) return NO;
    if (position.offset == _innerText.length && position.affinity == CSTextAffinityBackward) return NO;
    return YES;
}

/// Whether the range is valid (not out of bounds).
- (BOOL)_isTextRangeValid:(CSTextRange *)range {
...
- (CSTextPosition *)_correctedTextPosition:(CSTextPosition *)position {
    if (!position) return nil;
    if ([self _isTextPositionValid:position]) return position;
    if (position.offset < 0) {
        return [CSTextPosition positionWithOffset:0];
    }
    if (position.offset > _innerText.length) {
        return [CSTextPosition positionWithOffset:_innerText.length];
    }
    if (position.offset == 0 && position.affinity == CSTextAffinityBackward) {
        return [CSTextPosition positionWithOffset:position.offset];
    }
    if (position.offset == _innerText.length && position.affinity == CSTextAffinityBackward) {
        return [CSTextPosition positionWithOffset:position.offset];
    }
    return position;
}
...
- (CSTextRange *)_correctedTextRange:(CSTextRange *)range {
    if (!range) return nil;
    if ([self _isTextRangeValid:range]) return range;
    CSTextPosition *start = [self _correctedTextPosition:range.start];
    CSTextPosition *end = [self _correctedTextPosition:range.end];
    return [CSTextRange rangeWithStart:start end:end];
}
...
- (CGPoint)_convertPointToLayout:(CGPoint)point {
    ...
    if (_innerLayout.container.isVerticalForm) {
        ...
        if (boundingSize.width < self.bounds.size.width) {
            if (_textVerticalAlignment == CSTextVerticalAlignmentCenter) {
                point.x += (self.bounds.size.width - boundingSize.width) * 0.5;
            } else if (_textVerticalAlignment == CSTextVerticalAlignmentBottom) {
...
- (void)_replaceRange:(CSTextRange *)range withText:(NSString *)text notifyToDelegate:(BOOL)notify{
    if (NSEqualRanges(range.asRange, _selectedTextRange.asRange)) {
        ...
        _selectedTextRange = [CSTextRange rangeWithRange:newRange];
        ...
        if (range.asRange.length != text.length) {
            ...
            if (unionRange.length == 0) {
                ...
                _selectedTextRange = [CSTextRange rangeWithRange:NSMakeRange(range.start.offset + text.length, 0)];
...
- (void)_updateAttributesHolder {
    if (_innerText.length > 0) {
        ...
        [_typingAttributesHolder removeAttribute:CSTextBorderAttributeName range:NSMakeRange(0, _typingAttributesHolder.length)];
        [_typingAttributesHolder removeAttribute:CSTextBackgroundBorderAttributeName range:NSMakeRange(0, _typingAttributesHolder.length)];
    }
}
...
- (BOOL)_parseText {
    if (self.textParser) {
        CSTextRange *oldTextRange = _selectedTextRange;
        NSRange newRange = _selectedTextRange.asRange;
        ...
        CSTextRange *newTextRange = [CSTextRange rangeWithRange:newRange];
        ...
        switch (result.resultType) {
            ...
            case NSTextCheckingTypePhoneNumber: {
                ...
                if (_highlightTextAttributes.count) {
                    CSTextHighlight *highlight = [CSTextHighlight highlightWithAttributes:_highlightTextAttributes];
...
- (void)_resetUndoAndRedoStack {
    ...
    _CSTextViewUndoObject *object = [_CSTextViewUndoObject objectWithText:_innerText.copy range:_selectedTextRange.asRange];
...
- (void)_saveToUndoStack {
    ...
    _CSTextViewUndoObject *lastObject = _undoStack.lastObject;
...
- (void)_saveToRedoStack {
    if (!_allowsUndoAndRedo) return;
    _CSTextViewUndoObject *lastObject = _redoStack.lastObject;
    if ([lastObject.text isEqualToAttributedString:self.attributedText]) return;
...
- (BOOL)_canUndo {
    ...
    _CSTextViewUndoObject *object = _undoStack.lastObject;
...
- (void)_redo {
    ...
    _CSTextViewUndoObject *object = _redoStack.lastObject;
...
- (void)_setTextParser:(id<CSTextParser>)textParser {
...
- (void)_setLinePositionModifier:(id<CSTextLinePositionModifier>)linePositionModifier {
...
- (void)_initTextView {
    ...
    // UITextInput
    _selectedTextRange = [CSTextRange defaultRange];
    ...
    _innerContainer = [CSTextContainer new];
    ...
    CSTextHighlight *highlight = [CSTextHighlight new];
    CSTextBorder * border = [CSTextBorder new];
    ...
    _containerView = [CSTextContainerView new];
    _containerView.hostView = self;
    
    _selectionView = [CSTextSelectionView new];
    _selectionView.userInteractionEnabled = NO;
    _selectionView.hostView = self;
    _selectionView.color = [self _defaultTintColor];
    
    _magnifierCaret = [CSTextMagnifier magnifierWithType:CSTextMagnifierTypeCaret];
    _magnifierCaret.hostView = _containerView;
    _magnifierRanged = [CSTextMagnifier magnifierWithType:CSTextMagnifierTypeRanged];
    ...
    self.debugOption = [CSTextDebugOption sharedDebugOption];
    [CSTextDebugOption addDebugTarget:self];
    ...
    [[CSTextKeyboardManager defaultManager] addObserver:self];
...
- (void)dealloc {
    ...
    [[CSTextKeyboardManager defaultManager] removeObserver:self];
    
    [[CSTextEffectWindow sharedWindow] hideSelectionDot:_selectionView];
    [[CSTextEffectWindow sharedWindow] hideMagnifier:_magnifierCaret];
    [[CSTextEffectWindow sharedWindow] hideMagnifier:_magnifierRanged];
    
    [CSTextDebugOption removeDebugTarget:self];
...
- (void)scrollRangeToVisible:(NSRange)range {
    CSTextRange *textRange = [CSTextRange rangeWithRange:range];
    textRange = [self _correctedTextRange:textRange];
...
- (void)setText:(NSString *)text {
    ...
    [self replaceRange:[CSTextRange rangeWithRange:NSMakeRange(0, _innerText.length)] withText:text];
}
...
- (void)setTextParser:(id<CSTextParser>)textParser {
    if (_textParser == textParser || [_textParser isEqual:textParser]) return;
    [self _setTextParser:textParser];
    if (textParser && _text.length) {
        [self replaceRange:[CSTextRange rangeWithRange:NSMakeRange(0, _text.length)] withText:_text];
    }
    [self _resetUndoAndRedoStack];
    [self _commitUpdate];
}
...
- (void)setAttributedText:(NSAttributedString *)attributedText {
    ...
    if (text.length == 0) {
        [self replaceRange:[CSTextRange rangeWithRange:NSMakeRange(0, _innerText.length)] withText:@""];
        return;
    }
    ...
    _innerText = text;
    [self _parseText];
    _selectedTextRange = [CSTextRange rangeWithRange:NSMakeRange(0, _innerText.length)];
    [_inputDelegate textDidChange:self];
    [_inputDelegate selectionDidChange:self];
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:CSTextViewTextDidChangeNotification object:self];
...
- (void)setTextVerticalAlignment:(CSTextVerticalAlignment)textVerticalAlignment {
...
- (void)setSelectedRange:(NSRange)selectedRange {
    ...
    CSTextRange *range = [CSTextRange rangeWithRange:selectedRange];
    range = [self _correctedTextRange:range];
    [self _endTouchTracking];
...
- (void)setDebugOption:(CSTextDebugOption *)debugOption {
    _containerView.debugOption = debugOption;
}

- (CSTextDebugOption *)debugOption {
    return _containerView.debugOption;
}

- (CSTextLayout *)textLayout {
...
- (CGSize)sizeThatFits:(CGSize)size {
    if (!_verticalForm && size.width <= 0) size.width = CSTextContainerMaxSize.width;
    if (_verticalForm && size.height <= 0) size.height = CSTextContainerMaxSize.height;
    ...
    if (!_verticalForm) {
        size.height = CSTextContainerMaxSize.height;
    } else {
        size.width = CSTextContainerMaxSize.width;
    }
    
    CSTextContainer *container = [_innerContainer copy];
    container.size = size;
    
    CSTextLayout *layout = [CSTextLayout layoutWithContainer:container text:_innerText];
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (_state.trackingTouch) {
        ...
        if (_highlight) {
            ...
            CSTextHighlight *highlight = [self _getHighlightAtPoint:_trackingPoint range:NULL];
            ...
            if (_state.trackingCaret) {
                ...
                if (!_state.touchMoved) {
                    if (_state.selectedWithoutEdit) {
                        ...
                        if (self.isFirstResponder) {
                            CSTextRange *_oldRange = _trackingRange;
                            [self _updateTextRangeByTrackingCaret];
...
- (BOOL)becomeFirstResponder {
    ...
    if (!isFirstResponder && become) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:CSTextViewTextDidBeginEditingNotification object:self];
    }
    return become;
}
...
- (BOOL)resignFirstResponder {
    ...
    if (resign) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:CSTextViewTextDidEndEditingNotification object:self];
    }
    return resign;
}
...
- (void)paste:(id)sender {
    ...
    if (!atr && _allowsPasteImage) {
        UIImage *img = nil;
        if (p.GIFData) {
            img = [CSImage imageWithData:p.GIFData scale:kScreenScale];
        }
        if (!img && p.PNGData) {
            img = [CSImage imageWithData:p.PNGData scale:kScreenScale];
        }
        if (!img && p.WEBPData) {
            img = [CSImage imageWithData:p.WEBPData scale:kScreenScale];
        }
        if (!img) {
            img = p.image;
        }
        ...
        if (img && img.size.width > 1 && img.size.height > 1) {
            id content = img;
            if ([img conformsToProtocol:@protocol(CSAnimatedImage)]) {
                id<CSAnimatedImage> ani = (id)img;
                if (ani.animatedImageFrameCount > 1) {
                    CSAnimatedImageView *aniView = [[CSAnimatedImageView alloc] initWithImage:img];
                    if (aniView) {
                        content = aniView;
                    }
                }
    ...
    if (atr) {
        ...
        CSTextPosition *pos = [self _correctedTextPosition:[CSTextPosition positionWithOffset:endPosition]];
        CSTextRange *range = [_innerLayout textRangeByExtendingPosition:pos];
        range = [self _correctedTextRange:range];
        if (range) {
...
- (void)select:(id)sender {
    ...
    CSTextRange *newRange = [self _getClosestTokenRangeAtPosition:_selectedTextRange.start];
...
- (id<CSTextViewDelegate>)delegate {
    return _outerDelegate;
}

- (void)setDelegate:(id<CSTextViewDelegate>)delegate {
...
#pragma mark - @protocol CSTextKeyboardObserver
...
- (void)deleteBackward {
    ...
    if (!_state.deleteConfirm && range.length == 0 && range.location > 0) {
        ...
        CSTextBinding *binding = [_innerText attribute:CSTextBindingAttributeName atIndex:range.location - 1 longestEffectiveRange:&effectiveRange inRange:NSMakeRange(0, _innerText.length)];
        if (binding && binding.deleteConfirm) {
            _state.deleteConfirm = YES;
            [_inputDelegate selectionWillChange:self];
            _selectedTextRange = [CSTextRange rangeWithRange:effectiveRange];
            _selectedTextRange = [self _correctedTextRange:_selectedTextRange];
            [_inputDelegate selectionDidChange:self];
    ...
    _state.deleteConfirm = NO;
    if (range.length == 0) {
        CSTextRange *extendRange = [_innerLayout textRangeByExtendingPosition:_selectedTextRange.end inDirection:UITextLayoutDirectionLeft offset:1];
    ...
    [self replaceRange:[CSTextRange rangeWithRange:range] withText:@""];
}
...
- (void)setSelectedTextRange:(CSTextRange *)selectedTextRange {
...
- (void)setMarkedText:(NSString *)markedText selectedRange:(NSRange)selectedRange {
    ...
    if (_markedTextRange == nil) {
        _markedTextRange = [CSTextRange rangeWithRange:NSMakeRange(_selectedTextRange.end.offset, markedText.length)];
        [_innerText replaceCharactersInRange:NSMakeRange(_selectedTextRange.end.offset, 0) withString:markedText];
        _selectedTextRange = [CSTextRange rangeWithRange:NSMakeRange(_selectedTextRange.start.offset + selectedRange.location, selectedRange.length)];
    } else {
        ...
        _markedTextRange = [CSTextRange rangeWithRange:NSMakeRange(_markedTextRange.start.offset, markedText.length)];
        _selectedTextRange = [CSTextRange rangeWithRange:NSMakeRange(_markedTextRange.start.offset + selectedRange.location, selectedRange.length)];
    }
...
- (void)replaceRange:(CSTextRange *)range withText:(NSString *)text {
    if (!range) return;
    if (!text) text = @"";
...
- (void)setBaseWritingDirection:(UITextWritingDirection)writingDirection forRange:(CSTextRange *)range {
    if (!range) return;
    range = [self _correctedTextRange:range];
...
- (NSString *)textInRange:(CSTextRange *)range {
    range = [self _correctedTextRange:range];
    if (!range) return @"";
    return [_innerText.string substringWithRange:range.asRange];
}

- (UITextWritingDirection)baseWritingDirectionForPosition:(CSTextPosition *)position inDirection:(UITextStorageDirection)direction {
    [self _updateIfNeeded];
    position = [self _correctedTextPosition:position];
...
- (CSTextPosition *)beginningOfDocument {
    return [CSTextPosition positionWithOffset:0];
}

- (CSTextPosition *)endOfDocument {
    return [CSTextPosition positionWithOffset:_innerText.length];
}

- (CSTextPosition *)positionFromPosition:(CSTextPosition *)position offset:(NSInteger)offset {
    ...
    if (newLocation != 0 && newLocation != _innerText.length) {
        // fix emoji
        [self _updateIfNeeded];
        CSTextRange *extendRange = [_innerLayout textRangeByExtendingPosition:[CSTextPosition positionWithOffset:newLocation]];
    ...
    CSTextPosition *p = [CSTextPosition positionWithOffset:newLocation];
    return [self _correctedTextPosition:p];
}

- (CSTextPosition *)positionFromPosition:(CSTextPosition *)position inDirection:(UITextLayoutDirection)direction offset:(NSInteger)offset {
    [self _updateIfNeeded];
    CSTextRange *range = [_innerLayout textRangeByExtendingPosition:position inDirection:direction offset:offset];
    ...
    CSTextPosition *newPosition = forward ? range.end : range.start;
    if (newPosition.offset > _innerText.length) {
        newPosition = [CSTextPosition positionWithOffset:_innerText.length affinity:CSTextAffinityBackward];
    }
    
    return [self _correctedTextPosition:newPosition];
}

- (CSTextRange *)textRangeFromPosition:(CSTextPosition *)fromPosition toPosition:(CSTextPosition *)toPosition {
    return [CSTextRange rangeWithStart:fromPosition end:toPosition];
}

- (NSComparisonResult)comparePosition:(CSTextPosition *)position toPosition:(CSTextPosition *)other {
    return [position compare:other];
}

- (NSInteger)offsetFromPosition:(CSTextPosition *)from toPosition:(CSTextPosition *)toPosition {
    return toPosition.offset - from.offset;
}

- (CSTextPosition *)positionWithinRange:(CSTextRange *)range farthestInDirection:(UITextLayoutDirection)direction {
    NSRange nsRange = range.asRange;
    if (direction == UITextLayoutDirectionLeft | direction == UITextLayoutDirectionUp) {
        return [CSTextPosition positionWithOffset:nsRange.location];
    } else {
        return [CSTextPosition positionWithOffset:nsRange.location + nsRange.length affinity:CSTextAffinityBackward];
    }
}

- (CSTextRange *)characterRangeByExtendingPosition:(CSTextPosition *)position inDirection:(UITextLayoutDirection)direction {
    [self _updateIfNeeded];
    CSTextRange *range = [_innerLayout textRangeByExtendingPosition:position inDirection:direction offset:1];
    return [self _correctedTextRange:range];
}
...
- (CSTextPosition *)closestPositionToPoint:(CGPoint)point {
    [self _updateIfNeeded];
    point = [self _convertPointToLayout:point];
    CSTextPosition *position = [_innerLayout closestPositionToPoint:point];
    return [self _correctedTextPosition:position];
}

- (CSTextPosition *)closestPositionToPoint:(CGPoint)point withinRange:(CSTextRange *)range {
    CSTextPosition *pos = (id)[self closestPositionToPoint:point];
    if (!pos) return nil;
...
- (CSTextRange *)characterRangeAtPoint:(CGPoint)point {
    [self _updateIfNeeded];
    point = [self _convertPointToLayout:point];
    CSTextRange *r = [_innerLayout closestTextRangeAtPoint:point];
...
- (NSArray *)selectionRectsForRange:(CSTextRange *)range {
    ...
    [rects enumerateObjectsUsingBlock:^(CSTextSelectionRect *rect, NSUInteger idx, BOOL *stop) {
        rect.rect = [self _convertRectFromLayout:rect.rect];
    }];
    return rects;
}
...
- (UITextStorageDirection)selectionAffinity {
    if (_selectedTextRange.end.affinity == CSTextAffinityForward) {
...
- (void)setSelectionAffinity:(UITextStorageDirection)selectionAffinity {
    _selectedTextRange = [CSTextRange rangeWithRange:_selectedTextRange.asRange affinity:selectionAffinity == UITextStorageDirectionForward ? CSTextAffinityForward : CSTextAffinityBackward];
    [self _updateSelectionView];
}

- (NSDictionary *)textStylingAtPosition:(CSTextPosition *)position inDirection:(UITextStorageDirection)direction {
    if (!position) return nil;
...
- (CSTextPosition *)positionWithinRange:(CSTextRange *)range atCharacterOffset:(NSInteger)offset {
    if (!range) return nil;
    ...
    else return [CSTextPosition positionWithOffset:offset];
}

- (NSInteger)characterOffsetOfPosition:(CSTextPosition *)position withinRange:(CSTextRange *)range {
...
@interface CSTextView(IBInspectableProperties)
@end

@implementation CSTextView(IBInspectableProperties)
...
- (void)setDebugEnabled_:(BOOL)enabled {
    if (!enabled) {
        ...
        CSTextDebugOption *debugOption = [CSTextDebugOption new];
