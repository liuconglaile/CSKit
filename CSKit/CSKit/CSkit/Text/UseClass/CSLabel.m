...
static dispatch_queue_t CSLabelGetReleaseQueue() {
...
@interface CSLabel() <CSTextDebugTarget, CSAsyncLayerDelegate> {
    NSMutableAttributedString *_innerText; ///< nonnull
    CSTextLayout *_innerLayout;
    CSTextContainer *_innerContainer; ///< nonnull
    ...
    CSTextLayout *_shrinkInnerLayout;
    CSTextLayout *_shrinkHighlightLayout;
...
- (void)_updateLayout {
    _innerLayout = [CSTextLayout layoutWithContainer:_innerContainer text:_innerText];
    _shrinkInnerLayout = [CSLabel _shrinkLayoutWithLayout:_innerLayout];
}
...
- (void)_clearInnerLayout {
    ...
    CSTextLayout *layout = _innerLayout;
    _innerLayout = nil;
    _shrinkInnerLayout = nil;
    dispatch_async(CSLabelGetReleaseQueue(), ^{
...
- (CSTextLayout *)_highlightLayout {
    return _shrinkHighlightLayout ? _shrinkHighlightLayout : _highlightLayout;
}

+ (CSTextLayout *)_shrinkLayoutWithLayout:(CSTextLayout *)layout {
    if (layout.text.length && layout.lines.count == 0) {
        CSTextContainer *container = layout.container.copy;
        container.maximumNumberOfRows = 1;
        CGSize containerSize = container.size;
        if (!container.verticalForm) {
            containerSize.height = CSTextContainerMaxSize.height;
        } else {
            containerSize.width = CSTextContainerMaxSize.width;
        }
        container.size = containerSize;
        return [CSTextLayout layoutWithContainer:container text:layout.text];
    } else {
        return nil;
    }
}
...
- (void)_trackDidLongPress {
    ...
    if (_state.hasLongPressAction && _textLongPressAction) {
        ...
        CSTextRange *textRange = [self._innerLayout textRangeAtPoint:point];
    ...
    if (_highlight) {
        CSTextAction longPressAction = _highlight.longPressAction ? _highlight.longPressAction : _highlightLongPressAction;
        if (longPressAction) {
            CSTextPosition *start = [CSTextPosition positionWithOffset:_highlightRange.location];
            CSTextPosition *end = [CSTextPosition positionWithOffset:_highlightRange.location + _highlightRange.length affinity:CSTextAffinityBackward];
            CSTextRange *range = [CSTextRange rangeWithStart:start end:end];
...
- (CSTextHighlight *)_getHighlightAtPoint:(CGPoint)point range:(NSRangePointer)range {
    ...
    CSTextHighlight *highlight = [_innerText attribute:CSTextHighlightAttributeName
                                               atIndex:startIndex
...
- (void)_showHighlightAnimated:(BOOL)animated {
    ...
    if (!_highlightLayout) {
        ...
        _shrinkHighlightLayout = [CSLabel _shrinkLayoutWithLayout:_highlightLayout];
...
- (void)_updateOuterLineBreakMode {
    if (_innerContainer.truncationType) {
        switch (_innerContainer.truncationType) {
            case CSTextTruncationTypeStart: {
                _lineBreakMode = NSLineBreakByTruncatingHead;
            } break;
            case CSTextTruncationTypeEnd: {
                _lineBreakMode = NSLineBreakByTruncatingTail;
            } break;
            case CSTextTruncationTypeMiddle: {
...
- (void)_initLabel {
    ((CSAsyncLayer *)self.layer).displaysAsynchronously = NO;
    ...
    _debugOption = [CSTextDebugOption sharedDebugOption];
    [CSTextDebugOption addDebugTarget:self];
    
    _font = [self _defaultFont];
    _textColor = [UIColor blackColor];
    _textVerticalAlignment = CSTextVerticalAlignmentCenter;
    _numberOfLines = 1;
    _textAlignment = NSTextAlignmentNatural;
    _lineBreakMode = NSLineBreakByTruncatingTail;
    _innerText = [NSMutableAttributedString new];
    _innerContainer = [CSTextContainer new];
    _innerContainer.truncationType = CSTextTruncationTypeEnd;
...
- (CGSize)sizeThatFits:(CGSize)size {
    ...
    if ((!_verticalForm && size.width == self.bounds.size.width) ||
        ...
        CSTextLayout *layout = self._innerLayout;
        BOOL contains = NO;
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    ...
    [self _initLabel];
    CSTextContainer *innerContainer = [aDecoder decodeObjectForKey:@"innerContainer"];
    if (innerContainer) {
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (_state.trackingTouch) {
        ...
        if (_state.touchMoved && _highlight) {
            CSTextHighlight *highlight = [self _getHighlightAtPoint:point range:NULL];
    ...
    if (_state.trackingTouch) {
        ...
        if (_highlight) {
            if (!_state.touchMoved || [self _getHighlightAtPoint:point range:NULL] == _highlight) {
                CSTextAction tapAction = _highlight.tapAction ? _highlight.tapAction : _highlightTapAction;
                if (tapAction) {
                    CSTextPosition *start = [CSTextPosition positionWithOffset:_highlightRange.location];
                    CSTextPosition *end = [CSTextPosition positionWithOffset:_highlightRange.location + _highlightRange.length affinity:CSTextAffinityBackward];
                    CSTextRange *range = [CSTextRange rangeWithStart:start end:end];
...
- (void)setLineBreakMode:(NSLineBreakMode)lineBreakMode {
    ...
    switch (lineBreakMode) {
        ...
        case NSLineBreakByCharWrapping:
        case NSLineBreakByClipping: {
            _innerContainer.truncationType = CSTextTruncationTypeNone;
            _innerText.lineBreakMode = lineBreakMode;
        } break;
        case NSLineBreakByTruncatingHead:{
            _innerContainer.truncationType = CSTextTruncationTypeStart;
            _innerText.lineBreakMode = NSLineBreakByWordWrapping;
        } break;
        case NSLineBreakByTruncatingTail:{
            _innerContainer.truncationType = CSTextTruncationTypeEnd;
        ...
        case NSLineBreakByTruncatingMiddle: {
            _innerContainer.truncationType = CSTextTruncationTypeMiddle;
            _innerText.lineBreakMode = NSLineBreakByWordWrapping;
        } break;
        default: break;
    }
...
- (void)setTextLayout:(CSTextLayout *)textLayout {
...
- (void)setDisplaysAsynchronously:(BOOL)displaysAsynchronously {
    ...
    ((CSAsyncLayer *)self.layer).displaysAsynchronously = displaysAsynchronously;
}
...
- (CGSize)intrinsicContentSize {
    if (_preferredMaxLayoutWidth == 0) {
        CSTextContainer *container = [_innerContainer copy];
        container.size = CSTextContainerMaxSize;
...
#pragma mark - CSAsyncLayerDelegate
...
- (CSAsyncLayerDisplayTask *)newAsyncDisplayTask {
    ...
    CSTextContainer *container = _innerContainer;
    CSTextVerticalAlignment verticalAlignment = _textVerticalAlignment;
    CSTextDebugOption *debug = _debugOption;
    ...
    __block CSTextLayout *layout = (_state.showingHighlight && _highlightLayout) ? self._highlightLayout : self._innerLayout;
    __block CSTextLayout *shrinkLayout = nil;
    ...
    // create display task
    CSAsyncLayerDisplayTask *task = [CSAsyncLayerDisplayTask new];
    ...
    task.display = ^(CGContextRef context, CGSize size, BOOL (^isCancelled)(void)) {
        ...
        CSTextLayout *drawLayout = layout;
        if (layoutNeedUpdate) {
            layout = [CSTextLayout layoutWithContainer:container text:text];
            shrinkLayout = [CSLabel _shrinkLayoutWithLayout:layout];
            if (isCancelled()) return;
            layoutUpdated = YES;
        ...
        if (verticalAlignment == CSTextVerticalAlignmentCenter) {
        ...
        } else if (verticalAlignment == CSTextVerticalAlignmentBottom) {
            if (drawLayout.container.isVerticalForm) {
    ...
    task.didDisplay = ^(CALayer *layer, BOOL finished) {
        ...
        if (!finished) {
            ...
            for (CSTextAttachment *a in drawLayout.attachments) {
        ...
        __strong CSLabel *view = (CSLabel *)layer.delegate;
        if (!view) return;
...
@interface CSLabel(IBInspectableProperties)
@end

@implementation CSLabel (IBInspectableProperties)
