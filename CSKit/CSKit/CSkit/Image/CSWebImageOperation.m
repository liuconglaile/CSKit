...
static BOOL YYCGImageLastPixelFilled(CGImageRef image) {
    ...
    CGContextRef ctx = CGBitmapContextCreate(NULL, 1, 1, 8, 0, CSCGColorSpaceGetDeviceRGB(), kCGImageAlphaPremultipliedFirst | kCGBitmapByteOrderDefault);
    if (!ctx) return NO;
...
@interface CSWebImageOperation() <NSURLConnectionDelegate>
...
@property (nonatomic, strong) CSImageDecoder *progressiveDecoder;
...
@property (nonatomic, copy) CSWebImageProgressBlock progress;
@property (nonatomic, copy) CSWebImageTransformBlock transform;
@property (nonatomic, copy) CSWebImageCompletionBlock completion;
@end


@implementation CSWebImageOperation
...
///MARK: 全局映像请求网络线程,由NSURLConnection委托使用
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"CSWebImageOperation init error" reason:@"CSWebImageOperation must be initialized with a request. Use the designated initializer to init." userInfo:nil];
...
- (void)dealloc {
    ...
    if ([self isExecuting]) {
        ...
        if (_connection) {
            [_connection cancel];
            if (![_request.URL isFileURL] && (_options & CSWebImageOptionShowNetworkActivity)) {
                [[UIApplication sharedExtensionApplication] decrementNetworkActivityCount];
            }
        }
        if (_completion) {
            @autoreleasepool {
                _completion(nil, _request.URL, CSWebImageFromNone, CSWebImageStageCancelled, nil);
            }
        }
    }
    [_lock unlock];
...
- (void)_startOperation {
    ...
    @autoreleasepool {
        ...
        if (_cache &&
            ...
            UIImage *image = [_cache getImageForKey:_cacheKey withType:CSImageCacheTypeMemory];
            if (image) {
                [_lock lock];
                if (![self isCancelled]) {
                    if (_completion) _completion(image, _request.URL, CSWebImageFromMemoryCache, CSWebImageStageFinished, nil);
                }
                [self _finish];
                [_lock unlock];
                return;
            }
            if (!(_options & CSWebImageOptionIgnoreDiskCache)) {
                __weak typeof(self) _self = self;
                dispatch_async([self.class _imageQueue], ^{
                    __strong typeof(_self) self = _self;
                    if (!self || [self isCancelled]) return;
                    UIImage *image = [self.cache getImageForKey:self.cacheKey withType:CSImageCacheTypeDisk];
                    if (image) {
                        [self.cache setImage:image imageData:nil forKey:self.cacheKey withType:CSImageCacheTypeMemory];
...
- (void)_startRequest:(id)object {
    if ([self isCancelled]) return;
    @autoreleasepool {
        if ((_options & CSWebImageOptionIgnoreFailedURL) && URLBlackListContains(_request.URL)) {
            ...
            [_lock lock];
            if (![self isCancelled]) {
                if (_completion) _completion(nil, _request.URL, CSWebImageFromNone, CSWebImageStageFinished, error);
            }
            [self _finish];
            [_lock unlock];
            return;
...
- (void)_cancelOperation {
    @autoreleasepool {
        ...
        [_connection cancel];
        _connection = nil;
        if (_completion) _completion(nil, _request.URL, CSWebImageFromNone, CSWebImageStageCancelled, nil);
        [self _endBackgroundTask];
    }
}
...
- (void)_didReceiveImageFromDiskCache:(UIImage *)image {
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            if (image) {
                if (_completion) _completion(image, _request.URL, CSWebImageFromDiskCache, CSWebImageStageFinished, nil);
                [self _finish];
            } else {
...
- (void)_didReceiveImageFromWeb:(UIImage *)image {
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            if (_cache) {
                if (image || (_options & CSWebImageOptionRefreshImageCache)) {
                    NSData *data = _data;
                    dispatch_async([CSWebImageOperation _imageQueue], ^{
                        [_cache setImage:image imageData:data forKey:_cacheKey withType:CSImageCacheTypeAll];
                    });
                }
            }
            _data = nil;
            ...
            if (_completion) _completion(image, _request.URL, CSWebImageFromRemote, CSWebImageStageFinished, error);
            [self _finish];
        }
        [_lock unlock];
    }
...
- (void)connection:(NSURLConnection *)connection willSendRequestForAuthenticationChallenge:(NSURLAuthenticationChallenge *)challenge {
    @autoreleasepool {
        if ([challenge.protectionSpace.authenticationMethod isEqualToString:NSURLAuthenticationMethodServerTrust]) {
            if (!(_options & CSWebImageOptionAllowInvalidSSLCertificates) &&
...
- (NSCachedURLResponse *)connection:(NSURLConnection *)connection willCacheResponse:(NSCachedURLResponse *)cachedResponse {
    ...
    if (_options & CSWebImageOptionUseNSURLCache) {
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
    @autoreleasepool {
        ...
        /*--------------------------- progressive ----------------------------*/
        BOOL progressive = (_options & CSWebImageOptionProgressive) > 0;
        BOOL progressiveBlur = (_options & CSWebImageOptionProgressiveBlur) > 0;
        ...
        if (!_progressiveDecoder) {
            _progressiveDecoder = [[CSImageDecoder alloc] initWithScale:[UIScreen mainScreen].scale];
        }
        [_progressiveDecoder updateData:_data final:NO];
        if ([self isCancelled]) return;
        
        if (_progressiveDecoder.type == CSImageTypeUnknown ||
            _progressiveDecoder.type == CSImageTypeWebP ||
            _progressiveDecoder.type == CSImageTypeOther) {
            _progressiveDecoder = nil;
            _progressiveIgnored = YES;
            return;
        }
        if (progressiveBlur) { // only support progressive JPEG and interlaced PNG
            if (_progressiveDecoder.type != CSImageTypeJPEG &&
                _progressiveDecoder.type != CSImageTypePNG) {
                _progressiveDecoder = nil;
                _progressiveIgnored = YES;
                return;
            }
        ...
        if (!progressiveBlur) {
            CSImageFrame *frame = [_progressiveDecoder frameAtIndex:0 decodeForDisplay:YES];
            if (frame.image) {
                [_lock lock];
                if (![self isCancelled]) {
                    _completion(frame.image, _request.URL, CSWebImageFromRemote, CSWebImageStageProgress, nil);
            ...
            } else if (_progressiveDecoder.type == CSImageTypePNG) {
                if (!_progressiveDetected) {
                    NSDictionary *dic = [_progressiveDecoder framePropertiesAtIndex:0];
            ...
            if (image) {
                [_lock lock];
                if (![self isCancelled]) {
                    _completion(image, _request.URL, CSWebImageFromRemote, CSWebImageStageProgress, nil);
                    _lastProgressiveDecodeTimestamp = now;
                }
                [_lock unlock];
            }
    ...
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            ...
            dispatch_async([self.class _imageQueue], ^{
                ...
                BOOL shouldDecode = (self.options & CSWebImageOptionIgnoreImageDecoding) == 0;
                BOOL allowAnimation = (self.options & CSWebImageOptionIgnoreAnimatedImage) == 0;
                UIImage *image;
                BOOL hasAnimation = NO;
                if (allowAnimation) {
                    image = [[CSImage alloc] initWithData:self.data scale:[UIScreen mainScreen].scale];
                    if (shouldDecode) image = [image imageByDecoded];
                    if ([((CSImage *)image) animatedImageFrameCount] > 1) {
                        hasAnimation = YES;
                    }
                } else {
                    CSImageDecoder *decoder = [CSImageDecoder decoderWithData:self.data scale:[UIScreen mainScreen].scale];
                 ...
                 */
                CSImageType imageType = CSImageDetectType((__bridge CFDataRef)self.data);
                switch (imageType) {
                    case CSImageTypeJPEG:
                    case CSImageTypeGIF:
                    case CSImageTypePNG:
                    case CSImageTypeWebP: { // save to disk cache
                        if (!hasAnimation) {
                            if (imageType == CSImageTypeGIF ||
                                imageType == CSImageTypeWebP) {
            ...
            if (![self.request.URL isFileURL] && (self.options & CSWebImageOptionShowNetworkActivity)) {
                [[UIApplication sharedExtensionApplication] decrementNetworkActivityCount];
            }
        }
        [_lock unlock];
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error {
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            if (_completion) {
                _completion(nil, _request.URL, CSWebImageFromNone, CSWebImageStageFinished, error);
            }
            _connection = nil;
            _data = nil;
            if (![_request.URL isFileURL] && (_options & CSWebImageOptionShowNetworkActivity)) {
    ...
    @autoreleasepool {
        ...
        if ([self isCancelled]) {
            ...
            if (!_request) {
                ...
                if ((_options & CSWebImageOptionAllowBackgroundTask) && ![UIApplication isAppExtension]) {
