...
+ (CSImage *)imageWithContentsOfFile:(NSString *)path {
    return [[self alloc] initWithContentsOfFile:path];
}

+ (CSImage *)imageWithData:(NSData *)data {
    return [[self alloc] initWithData:data];
}

+ (CSImage *)imageWithData:(NSData *)data scale:(CGFloat)scale {
    return [[self alloc] initWithData:data scale:scale];
}
...
- (instancetype)initWithData:(NSData *)data scale:(CGFloat)scale {
    ...
    @autoreleasepool {
        ...
        CSImageFrame *frame = [decoder frameAtIndex:0 decodeForDisplay:YES];
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    NSNumber *scale = [aDecoder decodeObjectForKey:@"CSImageScale"];
    NSData *data = [aDecoder decodeObjectForKey:@"CSImageData"];
    if (data.length) {
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    if (_decoder.data.length) {
        [aCoder encodeObject:@(self.scale) forKey:@"CSImageScale"];
        [aCoder encodeObject:_decoder.data forKey:@"CSImageData"];
    } else {
