...
@implementation CSPhotoGroupView
...
- (void)presentFromImageView:(UIView *)fromView
    ...
    for (NSUInteger i = 0; i < self.groupItems.count; i++) {
        if (fromView == ((CSPhotoGroupItem *)self.groupItems[i]).thumbView) {
            page = (int)i;
            break;
        }
    }
    ...
    CSPhotoGroupCell *cell = [self cellForPage:self.currentPage];
    CSPhotoGroupItem *item = _groupItems[self.currentPage];
    
    if (!item.thumbClippedToTop) {
        NSString *imageKey = [[CSWebImageManager sharedManager] cacheKeyForURL:item.largeImageURL];
        if ([[CSWebImageManager sharedManager].cache getImageForKey:imageKey withType:CSImageCacheTypeMemory]) {
            cell.item = item;
        }
    }
    if (!cell.item) {
...
- (void)dismissAnimated:(BOOL)animated completion:(void (^)(void))completion {
    ...
    CSPhotoGroupCell *cell = [self cellForPage:currentPage];
    CSPhotoGroupItem *item = _groupItems[currentPage];
...
- (void)cancelAllImageLoad {
    [_cells enumerateObjectsUsingBlock:^(CSPhotoGroupCell *cell, NSUInteger idx, BOOL *stop) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    for (NSInteger i = page - 1; i <= page + 1; i++) { // preload left and right cell
        if (i >= 0 && i < self.groupItems.count) {
            CSPhotoGroupCell *cell = [self cellForPage:i];
            if (!cell) {
                CSPhotoGroupCell *cell = [self dequeueReusableCell];
                cell.page = i;
...
- (void)updateCellsForReuse {
    for (CSPhotoGroupCell *cell in _cells) {
        if (cell.superview) {
...
- (CSPhotoGroupCell *)dequeueReusableCell {
    CSPhotoGroupCell *cell = nil;
    for (cell in _cells) {
        if (!cell.superview) {
            return cell;
        }
...
- (CSPhotoGroupCell *)cellForPage:(NSInteger)page {
    for (CSPhotoGroupCell *cell in _cells) {
        if (cell.page == page) {
            return cell;
        }
...
- (void)doubleTap:(UITapGestureRecognizer *)g {
    ...
    CSPhotoGroupCell *tile = [self cellForPage:self.currentPage];
    if (tile) {
        if (tile.zoomScale > 1) {
...
- (void)longPress {
    ...
    CSImageType type = CSImageDetectType((__bridge CFDataRef)(imageItem));
    if (type != CSImageTypePNG &&
        type != CSImageTypeJPEG &&
        type != CSImageTypeGIF) {
    ...
    switch (g.state) {
        ...
        case UIGestureRecognizerStateEnded: {
            ...
            if (fabs(v.y) > 1000 || fabs(deltaY) > 120) {
                ...
                duration *= 0.8;
                duration = CS_CLAMP(duration, 0.05, 0.3);
