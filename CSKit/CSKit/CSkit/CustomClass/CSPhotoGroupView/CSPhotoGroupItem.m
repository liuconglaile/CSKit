...
@implementation CSPhotoGroupItem
...
- (id)copyWithZone:(NSZone *)zone {
    CSPhotoGroupItem *item = [self.class new];
    return item;
}
...
@implementation CSPhotoGroupCell
...
- (instancetype)init {
    ...
    _imageView = [CSAnimatedImageView new];
...
- (void)setItem:(CSPhotoGroupItem *)item {
    ...
    } transform:nil completion:^(UIImage *image, NSURL *url, CSWebImageFromType from, CSWebImageStage stage, NSError *error) {
        @strongify(self);
        if (!self) return;
        self.progressLayer.hidden = YES;
        if (stage == CSWebImageStageFinished) {
            self.maximumZoomScale = 3;
            if (image) {
