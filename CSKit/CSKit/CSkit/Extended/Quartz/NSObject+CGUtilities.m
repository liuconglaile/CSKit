...
CGContextRef CSCGContextCreateARGBBitmapContext(CGSize size, BOOL opaque, CGFloat scale) {
...
CGContextRef CSCGContextCreateGrayBitmapContext(CGSize size, CGFloat scale) {
    size_t width = ceil(size.width * scale);
    size_t height = ceil(size.height * scale);
    if (width < 1 || height < 1) return NULL;
...
CGAffineTransform CSCGAffineTransformGetFromPoints(CGPoint before[3], CGPoint after[3]) {
...
CGAffineTransform CSCGAffineTransformGetFromViews(UIView *from, UIView *to) {
    ...
    return CSCGAffineTransformGetFromPoints(before, after);
}
...
@implementation NSObject (CGUtilities)
